# Anleitung – Weighted_Optimization

Für jemanden, der diese Skripte nicht geschrieben hat. Technische Details
stehen in [README.md](README.md).

Was dieser Ordner rechnet: die **atom-gewichteten** Metriken. Uniformity und
Crosstalk werden über ein lokales Sub-Gitter an der Falle mit der thermischen
Aufenthaltsverteilung des Atoms gewichtet, statt über eine feste globale
Maske. Der Nachbarordner `Hard_Optimization` rechnet dieselben Größen hart
maskiert, `Combinated_Optimization` beides zugleich – und benutzt dabei den
Optimierer und die Plot-Klassen **aus diesem Ordner** mit (aus `lib/`).
...
* **Atom Weighting** – Masse, Temperatur und radiale Fallenfrequenz des
  Atoms; daraus folgt `sigma_atom`, die Breite der Gewichtungsfunktion. Die
  beiden Offset-Felder verschieben den Atomort als Bruchteil der
  Gesamtspannweite des Tonarrays (nicht des Pitch – das war eine frühere,
  korrigierte Konvention) und landen mit im Dateinamen.
...
Am Ende erscheinen die beiden Heatmaps (Uniformity_w, Crosstalk_w) und landen
als PNG in `Bilder/`. Der Datensatz liegt in `Results/`. Der Plot benutzt
immer die µm-Achse, unabhängig davon, wie der Scan-Bereich eingegeben wurde.
...
* **Intensity Grid** – Auflösung des globalen Intensitätsgitters
  (`n_grid`). Größer heißt genauer, aber quadratisch teurer. Für die
  atom-gewichteten Größen hängt das lokale Sub-Gitter an der Falle selbst und
  ist von `n_grid` unabhängig.
...
* **Leitgröße** und **Korridor** – nur im geführten Modus. Die Leitgerade ist
  der gewöhnliche lineare Fit einer anderen Größe (Default: Uniformity_w –
  ihr Talpfad ist der glatteste im Projekt); pro Spalte wird das lokale
  Minimum genommen, das ihr am nächsten liegt.
...
## `run_all_fits.py` – die drei Fit-Schritte

Etwas anderes als `run_plots.py`: hier stecken die eigentlichen Fits, und
konfiguriert wird über Konstanten oben im Skript, nicht über einen Dialog.

1. **Amplituden-Fit** (`RUN_AMPLITUDE_FIT`) – Flächen-Fit an r_x/r_y über dem
   (waist, width)-Gitter eines Amplituden-Scans.
2. **Waist-Width-Fit** (`RUN_WAIST_WIDTH_FIT`) – die Beziehung zwischen Waist
   und width entlang des Uniformity-Tals eines Fest-Amplituden-Scans. Das
   Modell ist per `FORCE_WAIST_WIDTH_MODEL` auf `"linear"` festgelegt; die
   Block-Kreuzvalidierung läuft weiter mit und steht informativ im Log.
   `None`/`"auto"` reaktiviert die automatische Modellwahl.
3. **Tal-Übersicht** (`DRAW_VALLEY_OVERVIEW`) – Übersichtsplot des Tals.

Jeder Schritt lässt sich einzeln abschalten. Die Ausgabenamen werden aus dem
tatsächlich verwendeten Dateinamen plus dem heutigen Datum gebildet, damit
ein Lauf mit einem anderen Datensatz keine älteren Ergebnisse überschreibt.

Der Talschnitt in `run_plots.py` ersetzt das **nicht**: er legt nur eine
Gerade durch den Talpfad, ohne Subpixel-Interpolation und ohne Modellwahl –
dafür schnell und in derselben Darstellung wie im
`Combinated_Optimization`-Ordner. Auf demselben Datensatz kommen beide auf
dieselbe Steigung in der 3. Stelle (0.28639 gegen 0.28709 MHz/µm).
