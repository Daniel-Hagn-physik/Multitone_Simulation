...
Scans und Auswertung der **atom-gewichteten** Metriken: Uniformity und
Crosstalk werden über ein lokales Sub-Gitter an der Falle mit der thermischen
Aufenthaltsverteilung des Atoms gewichtet (`sigma_atom` aus
`atom_temperature` und `trap_freq_r`) statt über eine feste globale Maske.

> Wie man die GUIs bedient, steht in **[ANLEITUNG.md](ANLEITUNG.md)** –
...
| die **drei Fits** (Amplituden, Waist-Width, Tal-Übersicht) rechnen | `run_all_fits.py` |
| die **Crosstalk-Streifen-Geometrie** zeichnen | `plot_crosstalk_stripe_geometry.py` |
| die **Airy-Ringe der Nachbarspots** zeichnen | `plot_neighbour_airy_rings.py` |
...
Weighted_Optimization/
    ...
    run_all_fits.py     <- ausführen: die drei Fit-Schritte
    plot_crosstalk_stripe_geometry.py   <- ausführen: Einzelgrafik
    plot_neighbour_airy_rings.py        <- ausführen: Einzelgrafik
    ...
    lib/                <- wird benutzt, nicht direkt ausgeführt
        ...
        weighted_multitone_flattop_optimizer.py           der Optimierer
        weighted_multitone_amplitude_dependence_plots.py  Scan-Plotter
        weighted_amp_scan_methods.py    die gewichteten Scan-Methoden
        fit_central_amplitudes.py       Schritt 1 von run_all_fits.py
        fit_waist_width_relation.py     Schritt 2 von run_all_fits.py
        fit_uniformity_valley_overview.py Schritt 3 von run_all_fits.py
...
`Hard_Optimization/`; alles, was die beiden Ordner unterscheidet
(Metrik-Familie, Dateinamens-Muster, Plot-Modul, Berichts-Präfix), steht in
`lib/paths.py`.

**`Combinated_Optimization` benutzt diesen Ordner mit**: dessen
`lib/paths.py` hängt `Weighted_Optimization/lib` in den `sys.path` und
importiert von dort den Optimierer, die Plot-Klassen, `scan_checkpoint`,
`perf_log`, `resume_picker` und `airy_scale` – nichts davon ist dort
dupliziert. Die alte, flache Struktur wird dabei weiterhin unterstützt (der
Ordner selbst als Fallback), damit eine ältere Kopie nicht bricht.

## Datensätze

| Datei | erzeugt von | enthält |
|---|---|---|
| `Results/scan_data_weighted_….pkl` | `run_scan.py` | `uniformity_weighted_grid`, `eta_weighted_grid`, `amps`, `best` |
| `Results/scan_amp_data_weighted_….pkl` | `run_amp_scan.py` | zusätzlich `r_x_grid`, `r_y_grid`, `r_bounds` |

`run_plots.py` erkennt beide Arten automatisch (`scan_data.dataset_kind()`).
Ein `scan_amp_data_weighted_*.pkl` ist außerdem die Eingangsdatei für
`Combinated_Optimization/run_hard_check.py`.

## Was `run_plots.py` erzeugt

Präfix `WeightedScan_N{Nx}x{Ny}_{n_win}x{n_width}pts_{Airy|Gauss}_{Datum}`:

| Datei | Fest-Amplituden | Amplituden-Scan |
|---|---|---|
| `…_metric_comparison.pdf` (Uniformity_w + Crosstalk_w) | x | x |
...
`run_plots.py` ist **kein Ersatz** für `run_all_fits.py`: dort stecken die
Flächen-Fits an r_x/r_y (`fit_central_amplitudes.py`) und die
Modellauswahl per Block-Kreuzvalidierung (`fit_waist_width_relation.py`).
Der Talschnitt in `run_plots.py` legt nur eine Gerade durch den Talpfad –
schnell, ohne Interpolation, und in derselben Darstellung wie im
`Combinated_Optimization`-Ordner.
...
**Plausibilitätsanker:** auf `scan_data_weighted_N3x4_40x40pts_Airy.pkl`
liefert die Talpfad-Gerade über der Uniformity
`width/MHz = 0.28639 · waist/µm + 0.002471` (R² = 0.9990). Derselbe Datensatz
ergibt in `fit_waist_width_relation.py` (Schritt 2 von `run_all_fits.py`)
`width = 0.28709 · waist + 0.00124`. Die Abweichung in der 3. Stelle ist der
Unterschied zwischen „direkt am Gitterpunkt abgelesen" (hier) und
„parabolisch subpixel-interpoliert" (dort) – kein Widerspruch.
...
* **Lokales Minimum nahe einer Leitgeraden** (Voreinstellung) – pro Spalte
  ...
  Uniformity_w – ihr Talpfad ist der glatteste im Projekt). **Sie wählt nur
  aus, sie verschiebt nichts** – die Punkte sind echte lokale Minima. Welcher
  Zweig verfolgt wird, entscheidet aber sie; das steht so auch im Bericht.
...
`lib/fit_waist_width_relation.py`, bewusst kopiert statt importiert (jenes
Modul setzt beim Import global `plt.rcParams` und legt Ordner an).
...
Die Metriken tragen überall den Index ihrer Familie: **U_w** und **η_w**
(atom-gewichtet). Das gilt in der Legende des Querschnitts, an den Colorbars, im
...
* Alle Hilfsmodule sind nach `lib/` gewandert; ausgeführt werden nur noch die
  `run_*.py` und die beiden `plot_*.py`. Inhaltlich sind die verschobenen
  Module unverändert – nur `_default_dir()` zeigt jetzt eine Ebene höher,
  damit `Results/` und `Bilder/` weiterhin neben den Skripten liegen und
  nicht in `lib/`.
* `weighted_winwidthscan_startdialog.py` → `run_scan.py`,
  `weighted_winwidthampscan_startdialog.py` → `run_amp_scan.py`.
* `run_plots.py` ist neu (vorher gab es nur
  `beispiel_weighted_amp_scan_ergebnisse_replotten.py` mit Konfig-Konstanten
  oben im Skript – das liegt jetzt zusammen mit
  `beispiel_weighted_amp_fit_abhaengigkeiten.py` in `Old_Scripts/`).
* `WeightedFixedScanPlotter` hat zusätzlich den Namen `plot_scan2d_combined`
  für seine Kombi-Methode bekommen (Alias auf
  `plot_scan2d_weighted_combined`), damit `lib/report.py` in beiden Ordnern
  dieselbe Datei sein kann. Der alte Name funktioniert unverändert.
* `Combinated_Optimization/lib/paths.py` wurde auf den neuen Pfad
  (`Weighted_Optimization/lib`) nachgezogen, mit Fallback auf die alte
  Struktur.
