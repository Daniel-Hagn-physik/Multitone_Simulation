...
# ======================================================================
# Atom-Gewichtung (thermische Ortsverteilung im harmonischen Fallenpotential)
# ======================================================================
# Standardmasse: Rubidium-85 (Default-Annahme fuer die gemessenen Fallen-
# parameter dieses Experiments: w0 = 0.86(7) um, pitch = 5.16(3) um,
# U = kB*273(75) uK, T = 17(2) uK, nu_r = 60.4(+2.4/-2.2) kHz @ 806 nm).
RB85_MASS = 84.911789738 * atomic_mass  # kg
...
def sigma_thermal(m, omega, T):
    ...
    Fuer hohe T (klassisches Regime) geht das in sigma^2 = kB*T/(m*omega^2)
    ueber; bei den hier typischen Werten (T ~ 17 uK, nu_r ~ 60 kHz) ist
    hbar*omega/(2*kB*T) << 1, die Korrektur traegt also nur auf der
    Sub-Prozent-Ebene bei, wird der Vollstaendigkeit halber aber
    mitgefuehrt.
    """
    x = hbar * omega / (2 * kB * T)
...
def atom_weight_2d(X, Y, center_x, center_y, sigma):
    """
    Unnormierte gaussfoermige Gewichtsfunktion W(x,y), die die thermische
    Aufenthaltswahrscheinlichkeit eines Atoms um eine Site (center_x,
    center_y) beschreibt. Die Normierung kuerzt sich in den unten
    definierten gewichteten Verhaeltnissen (Uniformity/Crosstalk) ohnehin
    heraus, daher wird hier bewusst NICHT normiert (spart eine Integration).
    """
...
def weighted_uniformity(I, W):
    """
    Gewichteter Variationskoeffizient (Analogon zu std(I)/mean(I) der
    harten Masken-Definition), aber mit kontinuierlicher Atom-Gewichtung W
    statt scharfem Rand:

        mu_w    = <I>_W  = sum(I*W) / sum(W)
        sigma_w = sqrt(<(I-mu_w)^2>_W)
        Uniformity_w = sigma_w / mu_w

    Fuer sigma_atom -> 0 (perfekt lokalisiertes Atom) geht W in eine
    Deltafunktion ueber und Uniformity_w -> 0 (kein Rauschen mehr, nur noch
    ein einzelner Punktwert) - ein physikalisch sinnvollerer Grenzfall als
    bei der harten Masken-Definition.
    """
    norm = np.sum(W)
    if norm == 0:
        return np.nan
...
def weighted_crosstalk(I_own, I_neighbor, W):
    """
    Gewichtetes Verhaeltnis Nachbar-/Eigenintensitaet (Analogon zu
    sum(I_neighbor)/sum(I_own) der harten Pitch-Box-Definition), aber
    gewichtet mit der Atom-Aufenthaltswahrscheinlichkeit W an der
    betrachteten Site statt einer geometrisch fest vorgegebenen Box:

        eta_w = sum(I_neighbor*W) / sum(I_own*W)

    Interpretation: "vom Atom im Mittel ueber seine thermische Verteilung
    gesehenes Verhaeltnis von Nachbar- zu Eigenlicht" - direkter Bezug zur
    tatsaechlich ungewollten Anregungswahrscheinlichkeit als eine feste,
    willkuerliche Box.
    """
...
class MultitoneFlatTopOptimizer:
    ...
    DEFAULTS = dict(
        ...
        # Default-Fallenparameter aus der Messung: w0 = 0.86(7) um,
        # pitch = 5.16(3) um, U = kB*273(75) uK, T = 17(2) uK,
        # nu_r = 60.4(+2.4/-2.2) kHz @ 806 nm, Atom = Rb-85.
        atom_mass=RB85_MASS,
    ...
    def _setup_geometry(self):
        ...
        # Thermische Atom-Ortsverteilung (fuer atom-gewichtete Uniformity/
        # Crosstalk, siehe _evaluate_weighted_metrics()). Wird bei jedem
        # set_parameters()-Aufruf neu berechnet, falls atom_mass/
        # atom_temperature/trap_freq_r geaendert wurden.
        self.omega_r = 2 * np.pi * self.trap_freq_r
        self.sigma_atom = sigma_thermal(self.atom_mass, self.omega_r, self.atom_temperature)
    ...
    # ------------------------------------------------------------------
    # Atom-gewichtete Uniformity/Crosstalk (kontinuierliche Gewichtung mit
    # der thermischen Atom-Ortsverteilung statt harter Masken)
    # ------------------------------------------------------------------
    def _build_local_weighted_grid(self, center_x, center_y):
        """
        Baut ein FEINES, lokal um (center_x, center_y) zentriertes Sub-Grid
        mit Ausdehnung +/- weighted_n_sigma * sigma_atom.

        Grund: self.sigma_atom liegt typischerweise bei ~100 nm, waehrend
        das globale self.X/self.Y-Grid ueber den ganzen sichtbaren Bereich
        (Pitch-Skala, mehrere um) mit self.n_grid Punkten aufgespannt ist -
        dessen Aufloesung reicht i.A. NICHT aus, um die schmale
        Gauss-Gewichtung W(x,y) numerisch sauber zu integrieren. Das
        globale Grid wird daher fuer die gewichteten Metriken nicht
        wiederverwendet, sondern durch dieses lokale Sub-Grid ersetzt.
        """
        half_extent = self.weighted_n_sigma * self.sigma_atom
        xs = np.linspace(center_x - half_extent, center_x + half_extent, self.weighted_n_grid)
        ys = np.linspace(center_y - half_extent, center_y + half_extent, self.weighted_n_grid)
        Xs, Ys = np.meshgrid(xs, ys)
        return Xs, Ys

    def _local_neighbor_intensity(self, X, Y, centers_x, centers_y, scale, amp_spots, profile_func):
        """
        ...
        erfassen (der per Definition ~pitch von der betrachteten Site
        entfernt liegt) - beim globalen self.X/self.Y-Grid ist das der
        Fall. Auf dem winzigen lokalen Sub-Grid für die atom-gewichteten
        Metriken (~weighted_n_sigma * sigma_atom breit, siehe
        _build_local_weighted_grid()) liegt der Nachbar-Peak dagegen weit
        AUSSERHALB des Fensters - np.max() sieht dort nur einen kleinen
        Schwanzwert, normiert faelschlich darauf und blaest die
        Intensitaet massiv (dort beobachtet: Faktor ~19) auf.

        Da das Profil translationsinvariant ist (die Summe hängt nur von
        Abständen zu den - verschobenen - Zentren ab), hat jede
        Nachbarkopie exakt dieselbe Peak-Intensität wie die eigene Site.
        Die Division entfällt daher ersatzlos; stattdessen wird konsistent
        mit demselben globalen norm_factor normiert wie I_own (siehe
        _evaluate_weighted_metrics()/_evaluate_weighted_only()).
        """
        ...
        for ix in (-1, 0, 1):
            for iy in (-1, 0, 1):
                if ix == 0 and iy == 0:
                    continue
                shifted_x = centers_x + ix * self.pitch
                shifted_y = centers_y + iy * self.pitch
                I_neighbor += profile_func(X, Y, shifted_x, shifted_y, scale, amp_spots)
        return I_neighbor

    def _evaluate_weighted_metrics(self, centers_x, centers_y, r_center, scale, amp_spots, profile_func,
                                    norm_factor):
        """
        Berechnet die atom-gewichteten Analoga von Uniformity/Crosstalk
        (siehe weighted_uniformity(), weighted_crosstalk()) auf einem
        lokalen, fein aufgeloesten Sub-Grid um die betrachtete Site
        (r_center, r_center).

        norm_factor: derselbe Normierungsfaktor (np.max(I_new) auf dem
        GLOBALEN Grid) wie in _evaluate() verwendet, damit
        uniformity_weighted/eta_weighted direkt mit den harten
        Masken-Werten (uniformity/eta) vergleichbar bleiben, statt sich
        auf ein moeglicherweise leicht abweichendes lokales Maximum zu
        normieren.
        ...
        return dict(
            ...
            eta_weighted=weighted_crosstalk(I_own_sub, I_neighbor_sub, W),
            sigma_atom=self.sigma_atom,
        )

    def _evaluate_weighted_only(self, win_val, width_val, amps=None):
        """
        Schlanke, schnelle Variante: berechnet NUR die atom-gewichteten
        Metriken (uniformity_weighted, eta_weighted) auf dem lokalen
        Sub-Grid - OHNE das teure globale self.X/self.Y-Grid, die harten
        Masken oder I_neighbor auf dem globalen Grid aufzubauen.

        Begründung: uniformity_weighted (sigma_w/mu_w) und eta_weighted
        (Verhältnis zweier Summen) sind beide skaleninvariant gegenüber
        einer gemeinsamen Normierung von I_own/I_neighbor - die (auf dem
        globalen Grid teure) Normierung aus _evaluate() spielt für den
        Wert dieser beiden Metriken also keine Rolle und wird hier
        übersprungen (norm_factor=1). Das macht diese Funktion um
        Größenordnungen schneller als _evaluate(..., weighted=True) und
        damit die richtige Wahl für eine Optimierungsschleife wie
        optimize_combined_weighted().

        Gibt None zurück bei ungültigen Parametern (analog zu _evaluate()).
        """
        if win_val <= 0:
            return None
        ...
        return self._evaluate_weighted_metrics(
            centers_x, centers_y, r_center, scale, amp_spots, profile_func, norm_factor=1.0
        )

    def weighted_crosstalk_breakdown(self, win_val, width_val, amps=None):
        """
        Zerlegt eta_weighted in die Beiträge der einzelnen 8 Nachbar-Richtungen,
        gruppiert nach Kanten-Nachbarn (oben/unten/links/rechts, direkt an der
        Site angrenzend, Abstand=pitch) und Diagonal-Nachbarn (Ecken, Abstand
        =sqrt(2)*pitch).

        Nützlich als Diagnose/Vertrauens-Check: ein auf den ersten Blick hoch
        wirkender eta_weighted-Gesamtwert kann legitim aus der SUMME vieler
        kleiner Einzelbeiträge stammen (Intensitäten addieren sich immer -
        anders als Feldamplituden gibt es keine destruktive Interferenz
        zwischen den Nachbar-Kopien), statt aus einem einzelnen, fehlerhaft
        überhöhten Beitrag. 'total' hier muss exakt mit
        _evaluate_weighted_only(...)['eta_weighted'] übereinstimmen.

        Gibt ein dict zurück mit:
        - 'total': Summe über alle 8 Nachbarn (== eta_weighted)
        - 'edge_sum'/'edge_per_neighbor': Summe/Mittelwert der 4 Kanten-Nachbarn
        - 'diag_sum'/'diag_per_neighbor': Summe/Mittelwert der 4 Diagonal-Nachbarn
        - 'per_direction': dict {(ix,iy): Einzelbeitrag dieser einen Nachbarkopie},
          (ix,iy) in {-1,0,1}^2 \\ {(0,0)}, z.B. (1,0)=rechts, (1,1)=oben-rechts
        Gibt None zurück bei ungültigen Parametern (analog zu _evaluate()).
        """
        if win_val <= 0:
            return None
        ...
        for ix in (-1, 0, 1):
            for iy in (-1, 0, 1):
                ...
                shifted_x = centers_x + ix * self.pitch
                shifted_y = centers_y + iy * self.pitch
                I_dir = profile_func(Xs, Ys, shifted_x, shifted_y, scale, amp_spots)
        ...
        edge_sum = sum(per_direction[k] for k in edge_keys)
        diag_sum = sum(per_direction[k] for k in diag_keys)

        return dict(
            total=edge_sum + diag_sum,
            edge_sum=edge_sum, edge_per_neighbor=edge_sum / len(edge_keys),
            diag_sum=diag_sum, diag_per_neighbor=diag_sum / len(diag_keys),
    ...
    def _evaluate(self, win_val, width_val, amps=None, grid=None, weighted=None):
        """
        ...
        weighted: ob zusaetzlich die atom-gewichteten Metriken
        ('uniformity_weighted', 'eta_weighted', siehe
        _evaluate_weighted_metrics()) berechnet werden sollen. Per Default
        (None) greift self.weighted_metrics_enabled - explizit True/False
        ueberschreibt das pro Aufruf. Kostet eine zusaetzliche, aber sehr
        billige lokale Sub-Grid-Auswertung (weighted_n_grid x
        weighted_n_grid Punkte, per Default 241x241) und ist daher in
        Scans/Optimierungsschleifen standardmaessig AUS, um diese nicht
        unnoetig zu verlangsamen.
        ...
        do_weighted = self.weighted_metrics_enabled if weighted is None else weighted
        if do_weighted:
            weighted_res = self._evaluate_weighted_metrics(
                centers_x, centers_y, r_center, scale, amp_spots, profile_func,
                norm_factor=norm_factor,
            )
            if weighted_res is not None:
                result.update(weighted_res)
            else:
                result.update(uniformity_weighted=np.nan, eta_weighted=np.nan, sigma_atom=self.sigma_atom)
    ...
    # ------------------------------------------------------------------
    def compute_initial(self, verbose=True, weighted=True):
        """
        Berechnet Intensitätsverteilung, Uniformity und Crosstalk für die
        aktuellen Eingangsparameter.

        weighted: berechnet zusätzlich die atom-gewichteten Metriken
        (uniformity_weighted, eta_weighted, siehe _evaluate_weighted_metrics()).
        Default True, da compute_initial() (anders als Scans/Optimierung)
        nur einmal aufgerufen wird - die zusätzliche Sub-Grid-Auswertung
        fällt hier nicht ins Gewicht.
        """
        details = self._evaluate(self.win, self.width, weighted=weighted)
        if details is None:
        ...
        if verbose:
            ...
            print(f"Uniformity (σ/μ, harte Maske): {details['uniformity']:.4f} ({details['uniformity']*100:.2f}%)")
            print(f"Überlapp der Nachbarn in die Zentralstruktur (harte Maske): {details['eta']*100:.3f}%")
            if weighted and np.isfinite(details.get('uniformity_weighted', np.nan)):
                print(f"  -> Atom-gewichtet (sigma_atom = {details['sigma_atom']*1e9:.1f} nm):")
                print(f"     Uniformity_w = {details['uniformity_weighted']:.4f} "
                      f"({details['uniformity_weighted']*100:.2f}%)")
                print(f"     Crosstalk_w  = {details['eta_weighted']*100:.4f}%")
    ...
    # ------------------------------------------------------------------
    # Vierte Stufe: win + width + Amplituden gegen die ATOM-GEWICHTETEN
    # Metriken (uniformity_weighted, eta_weighted) optimieren
    # ------------------------------------------------------------------
    def optimize_combined_weighted(self, alpha=0.9, x0=None, bounds=None, show=True, save=True, verbose=True):
        """
        Wie optimize_combined(), aber die Zielfunktion nutzt die ATOM-
        GEWICHTETEN Metriken (uniformity_weighted, eta_weighted, siehe
        _evaluate_weighted_only()) statt der harten Masken-Werte
        (uniformity, eta).

        Motivation: Bei sigma_atom << pitch/waist (wie bei den gemessenen
        Fallenparametern: sigma_atom ~ 108 nm, pitch = 5.16 µm) muss die
        harte, box-gemittelte Optimierung (optimize_combined()) nicht
        zwangsläufig auch den für das Atom tatsächlich relevanten Punkt
        (den exakten Zentrumspunkt) optimal machen - eine box-gemittelte
        Metrik kann lokale Strukturen genau dort verdecken. Diese Stufe
        optimiert direkt gegen die Größe, die für das Atom zählt.

        Nutzt dieselbe Startwert-/Bounds-Logik wie optimize_combined()
        (per set_initial_guess() hinterlegte Vermutung > Ergebnisse von
        optimize_win()/optimize_width()/optimize_combined() > aktuelles
        self.win/self.width, Amplituden = 1).

        Deutlich schneller pro Funktionsauswertung als optimize_combined(),
        da _evaluate_weighted_only() nur das lokale, feine Sub-Grid
        auswertet statt des vollen globalen self.X/self.Y-Grids.

        Ergebnis landet in self.results['combined_opt_weighted'] (inkl.
        der vollen Auswertung von _evaluate(..., weighted=True) am
        gefundenen Optimum, also auch mit den harten Metriken zum
        Vergleich sowie 'I'/'I_neighbor' fürs Plotten).
        """
        win0 = self.results.get('win_opt', {}).get('win_optimal',
                 self.results.get('combined_opt', {}).get('win_optimal', self.win))
        width0 = self.results.get('width_opt', {}).get('width_optimal',
                   self.results.get('combined_opt', {}).get('width_optimal', self.width))
        amp_x0 = np.ones(self.N_x)
        amp_y0 = np.ones(self.N_y)
        ...
        def objective(p):
            val = self._evaluate_weighted_only(p[0], p[1], amps=p[2:])
            if val is None:
                return 1e10
            uw, ew = val['uniformity_weighted'], val['eta_weighted']
            if not (np.isfinite(uw) and np.isfinite(ew)):
                return 1e10
            return alpha * uw + (1 - alpha) * ew

        if verbose:
            print("\n" + "=" * 60)
            print("VIERTE ITERATION (gewichtet): win, width, Amplituden gegen atom-gewichtete Metriken")
            print("=" * 60)
            print("\nStarte gewichtete kombinierte Optimierung...")
        ...
        win_opt, width_opt = result.x[0], result.x[1]
        amps_opt = result.x[2:]
        # Volle Auswertung (inkl. harter Metriken + Plot-Daten) am gefundenen Optimum
        details = self._evaluate(win_opt, width_opt, amps=amps_opt, weighted=True)
        ...
        if verbose:
            ...
            print(f"Optimierte amp_x: {amps_opt[:self.N_x]}")
            print(f"Optimierte amp_y: {amps_opt[self.N_x:]}")
            print(f"Uniformity_w: {details['uniformity_weighted']:.4f} ({details['uniformity_weighted']*100:.2f}%)")
            print(f"Crosstalk_w:  {details['eta_weighted']:.4f} ({details['eta_weighted']*100:.3f}%)")
            print(f"(zum Vergleich, harte Metriken an diesem Punkt: "
                  f"Uniformity={details['uniformity']*100:.2f}%, Crosstalk={details['eta']*100:.3f}%)")
            print(f"Funktionsauswertungen (nfev): {result.nfev}, Iterationen (nit): {result.nit}")

        self.results['combined_opt_weighted'] = dict(win_optimal=win_opt, width_optimal=width_opt,
                                                       nfev=result.nfev, nit=result.nit, **details)

        if show or save:
            self.plot_combined_optimized_weighted(show=show, save=save)

        return result.x

    def plot_combined_optimized_weighted(self, show=True, save=True):
        if 'combined_opt_weighted' not in self.results:
            raise RuntimeError("optimize_combined_weighted() muss zuerst aufgerufen werden.")
        res = self.results['combined_opt_weighted']
        tag = self._filetag()

        self._plot_intensity(
            res,
            title=(f"win+width+Amp (gewichtet) optimized: win = {res['win_optimal']:.3e} m, "
                   f"width = {res['width_optimal']:.3e} Hz, "
                   f"Uniformity_w = {res['uniformity_weighted']*100:.2f}%, "
                   f"Crosstalk_w = {res['eta_weighted']*100:.2f}%"),
            filename=f"FlatMultiTone_CombinedOptimizedWeighted_{tag}.png", show=show, save=save,
        )
        self._plot_neighbor(
            res, title=f"Neighbors (win+width+Amp, gewichtet-optimiert): Crosstalk_w = {res['eta_weighted']*100:.3f}%",
            filename=f"FlatMultiTone_NeighborCombinedOptimizedWeighted_{tag}.png", show=show, save=save,
        )
    ...
    def summary(self):
        ...
        def _line(label, r):
            line = f"{label:<22} Uniformity = {r['uniformity']*100:.2f}%, Crosstalk = {r['eta']*100:.3f}%"
            if np.isfinite(r.get('uniformity_weighted', np.nan)):
                line += (f"  |  Uniformity_w = {r['uniformity_weighted']*100:.2f}%, "
                         f"Crosstalk_w = {r['eta_weighted']*100:.3f}%")
            print(line)

        if 'initial' in self.results:
            _line("Original:", self.results['initial'])
        if 'win_opt' in self.results:
            _line("win-optimiert:", self.results['win_opt'])
        if 'width_opt' in self.results:
            _line("width-optimiert:", self.results['width_opt'])
        if 'combined_opt' in self.results:
            _line("win+width+amp-optim.:", self.results['combined_opt'])
        if 'combined_opt_weighted' in self.results:
            _line("win+width+amp (gew.):", self.results['combined_opt_weighted'])
        print("=" * 60)
        if not any(np.isfinite(self.results.get(k, {}).get('uniformity_weighted', np.nan))
                   for k in ('initial', 'win_opt', 'width_opt', 'combined_opt', 'combined_opt_weighted')):
            print("(Atom-gewichtete Metriken nicht berechnet - dazu compute_initial(weighted=True) bzw.\n"
                  " _evaluate(..., weighted=True) verwenden, oder self.weighted_metrics_enabled=True setzen.)")
