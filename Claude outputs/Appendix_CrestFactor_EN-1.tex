...
% ---------------------------------------------------------------------
%  Bibliography entries. Copy these into your own .bib file; every one
%  has been checked against Crossref / the publisher page.
% ---------------------------------------------------------------------
\begin{filecontents*}{appendix_refs.bib}
...
\paragraph{On sources.} Two kinds of statement appear below. The
\emph{starting points} are taken from the literature and cited: the
definition and bounds of the crest factor \cite{schroeder1970,boyd1986}, the
back-off of a compression-limited amplifier
\cite{cripps2006,pozar2012}, the acousto-optic diffraction law
\cite{salehteich2019,aaoptonotes}, the coupled-wave formalism
\cite{kogelnik1969,laude2003}, its multifrequency use
\cite{antonov2007}, and the device data \cite{aaaod,minicircuits}. The
\emph{derivations} that follow from them -- the centring criterion, the
$N=3$ result, and the crest-to-efficiency relation of
Sec.~\ref{sec:price} -- are carried out here in full and are self-contained;
they are cited only where they rest on one of the above.
...
\begin{equation}
  ...
  \label{eq:crestdef}
\end{equation}
This is the standard definition for multitone signals
\cite{schroeder1970,boyd1986}; in communications engineering the square
$\crest^2$ is the peak-to-average power ratio. It is a dimensionless
property of the \emph{waveform}: how far the
...
the lower bound would require $|A|$ to be constant. Which phase sets
approach the lower bound is a classical question: Schroeder's quadratic
phases $\varphi_n = -\pi n^2/N$ reach $\crest\approx1.7$ for large $N$
\cite{schroeder1970}, and the attainable minimum has been studied
systematically in \cite{boyd1986}. \emph{None of these sets satisfies the
centring criterion of Sec.~\ref{sec:centring}}, which is the entire tension
...
\emph{instantaneous voltage} \cite[Sec.~10.3]{pozar2012}; it does not care about the time-averaged
...
Backing a multitone transmitter off by its peak-to-average ratio is standard
practice in power-amplifier design \cite{cripps2006}; Eq.~\eqref{eq:pavglimit}
is that rule written for this chain.
...
\noindent With the amplifier used here \cite{minicircuits},
$P_{1\mathrm{dB}} = \SI{3.98}{\watt}$,
...
$\sigma\approx\SI{108}{\nano\meter}$; the thermal position distribution in
a tweezer and its measurement are described in \cite{tuchendler2008}. What harms it is not that the
...
index modulation \cite[Ch.~20]{salehteich2019}
\begin{equation}
...
electro-acoustic conversion efficiency $\zeta$. Equation~\eqref{eq:etasingle}
is the standard Bragg-regime result \cite[Ch.~20]{salehteich2019} and is
quoted in exactly this form in the manufacturer's application notes
\cite{aaoptonotes}. None of the four is known
...
sheet \cite{aaaod} ($\SI{58}{\percent}$ in order "1-1" at \SI{1.3}{\watt} per
axis, so
...
and never includes \Au. This is the coupled-wave ansatz of \cite{kogelnik1969}, extended from one
grating to $N$; a general treatment of the acousto-optic coupled-wave
equations is given in \cite{laude2003}. Substituting into the Helmholtz
equation with
...
crystal. For $\Delta k\neq0$ the two-wave solution acquires the detuned form
$\eta(\xi) = \nu^2(\nu^2+\xi^2)^{-1}\sin^2\sqrt{\nu^2+\xi^2}$ with
$\nu = \kappa\sqrt{P}$ and $\xi = \Delta k\,L/2$ \cite{kogelnik1969};
calibrating $\xi$ against the specified \SI{36}{\mega\hertz} bandwidth
\cite{aaaod} (half efficiency at the band edge) makes the \SI{0.37}{\mega\hertz}
tone span of the working point worth \SI{0.007}{\percent} in efficiency,
while an \SI{18}{\mega\hertz} span would already cost \SI{15}{\percent}.
...
\paragraph{Relation to the literature.} Multifrequency Bragg diffraction with
optimised tone phases has been treated experimentally and theoretically by
Antonov and Rezvov \cite{antonov2007}, who split one beam into seven equal
channels at \SI{80}{\percent} net efficiency; the same device class underlies
the multitone AOD tweezer arrays of \cite{endres2016}. The point relevant
...
\begin{empheq}[box=\fbox]{equation}
  ...
  \label{eq:etacrest}
\end{empheq}
With $\kappa = \SI{0.930}{\per\sqrt\watt}$ from Eq.~\eqref{eq:kappa}
\cite{aaaod} and $P_{1\mathrm{dB}} = \SI{3.98}{\watt}$ \cite{minicircuits}
the numerator is
...
\clearpage
\section*{BibTeX source}
\addcontentsline{toc}{section}{BibTeX source}
\noindent Copy the following into your own \texttt{.bib} file. Every entry
was checked against Crossref or the publisher's page; the two data sheets are
manufacturer documents.
\scriptsize
\verbatiminput{appendix_refs.bib}
\normalsize
