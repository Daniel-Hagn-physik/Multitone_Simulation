...
\paragraph{Die Einhüllende der zentrierten Familie ist reell.}
Wie bei $N=3$ rechnet man direkt. Mit $f_\mathrm{c}=\bar f$ liegen die vier
Töne bei $\hat n f_\mathrm{env}$, $\hat n\in\{-\tfrac32,-\tfrac12,\tfrac12,\tfrac32\}$.
Setze $u := \pi f_\mathrm{env} t$; eine Periode $T_\mathrm{env}$ entspricht
$u\in[0,\pi)$. Die Phasen schreibt man wie in Abschnitt~\ref{sec:crestback}
als $\varphi_n = c/2+\chi_n$ mit $\chi_{3-n}=-\chi_n$, hier
$\chi_3 = c/2 =: \chi_\mathrm{a}$ (außen) und
$\chi_2 = c/2-b =: \chi_\mathrm{i}$ (innen). Die Amplituden sind
$[\varrho,1,1,\varrho]$ mit $\varrho=\sqrt{r_y}$. Dann
\begin{align}
  A(t)\,e^{-\mathrm{i}c/2}
  &= \varrho\,e^{-\mathrm{i}(3u+\chi_\mathrm{a})}
   ...
   + e^{+\mathrm{i}(u+\chi_\mathrm{i})}
   + \varrho\,e^{+\mathrm{i}(3u+\chi_\mathrm{a})}\\
  &= 2\varrho\cos(3u+\chi_\mathrm{a}) + 2\cos(u+\chi_\mathrm{i}) .
\end{align}
Jetzt die Zeitverschiebung $u\to u-\chi_\mathrm{i}$, die nach dem ersten
Absatz das Maximum nicht ändert:
\begin{empheq}[box=\fbox]{equation}
  A\,e^{-\mathrm{i}c/2} \;=\; 2\varrho\cos(3u+\gamma) + 2\cos u,
  \qquad
  \gamma := \chi_\mathrm{a}-3\chi_\mathrm{i} = 3b-c = -\beta_0 .
  \label{eq:n4env}
\end{empheq}
Hier sieht man, warum bei $N=4$ ein Parameter überlebt und bei $N=3$ keiner.
Eine Zeitverschiebung verschiebt $u$ und $3u$ \emph{verschieden stark}. Man
kann damit die Phase des inneren Paares wegschieben, aber nicht gleichzeitig
die des äußeren. Bei $N=3$ gab es nur ein Paar, und $\chi$ verschwand
vollständig.

\paragraph{Die beiden Grenzfälle.}
\emph{Rampe}, $\gamma=0$: Bei $u=0$ stehen beide Kosinus auf $1$,
\begin{equation}
  \max|A| = 2\varrho+2 = \textstyle\sum_n a_n = 4{,}1540659,
  \qquad \crest_y = 2{,}8264839 .
\end{equation}
\emph{Arbeitspunkt}, $\gamma=\pi$: Das äußere Paar steht bei $u=0$ in
Gegenphase zum inneren. Mit $v:=\cos u\in[-1,1]$ und
$\cos 3u = 4v^3-3v$ wird Gl.~\eqref{eq:n4env} ein Polynom in $v$:
\begin{equation}
  A\,e^{-\mathrm{i}c/2} = 2v - 2\varrho\,(4v^3-3v) = (2+6\varrho)\,v - 8\varrho\,v^3 =: G(v) .
\end{equation}
$G$ ist ungerade, also reicht $v\ge0$. Nullsetzen der Ableitung,
$G'(v) = 2+6\varrho - 24\varrho v^2 = 0$, gibt
\begin{equation}
  v_*^2 = \frac{1+3\varrho}{12\varrho},
  \qquad
  G(v_*) = v_*\bigl[(2+6\varrho) - 8\varrho\,v_*^2\bigr]
         = v_*\cdot\tfrac43(1+3\varrho) ,
\end{equation}
und damit
\begin{empheq}[box=\fbox]{equation}
  ...
  \label{eq:n4closed}
\end{empheq}
Gültig, solange $v_*\le1$, also $\varrho\ge1/9$; der Randwert
$|G(1)| = |2-2\varrho|$ ist dann kleiner. Eingesetzt mit
$\varrho=\sqrt{1{,}16}=1{,}0770330$:
\begin{center}
\begin{tabular}{@{}lr@{}}
  \toprule
  $1+3\varrho$ & $4{,}2310991$\\
  $v_* = \cos u_*$ & $0{,}5721652$ \quad($u_* = 55{,}1^\circ$)\\
  $\max|A|$ & $3{,}2278503$\\
  $\sqrt{\sum a_n^2} = \sqrt{2r_y+2}$ & $2{,}0784610$\\
  $\crest_y$ & $\mathbf{2{,}1962739}$\\
  \bottomrule
\end{tabular}
\end{center}
