...
\begin{table}[htb]
  ...
  \begin{tabular}{@{}p{0.09\textwidth}p{0.38\textwidth}p{0.45\textwidth}@{}}
    ...
    $f_\mathrm{env}$ & ggT der Tonabstände; $1/f_\mathrm{env}$ ist die Periode von $|A|$ und das Mittelungsfenster (Abschnitt~\ref{sec:fenv}) &
...
\begin{equation}
  ...
  \label{eq:carrierrest}
\end{equation}
beschränkt; am Arbeitspunkt mit $T=\SI{5.405}{\micro\second}$ (eine Periode $T_\mathrm{env}$ der Einhüllenden, Abschnitt~\ref{sec:fenv}) sind das
...
\subsection{Was \boldmath$f_\mathrm{env}$ genau ist}\label{sec:fenv}

Die Einhüllende $A(t)$ aus Gl.~\eqref{eq:envdef} ist \emph{langsam}, aber
wie langsam genau, und mit welcher Periode wiederholt sie sich? Die Antwort
ist die Frequenz $f_\mathrm{env}$, und aus ihr folgt, über welches Fenster
der Effektivwert zu bilden ist.

\paragraph{Definition.}
\begin{empheq}[box=\fbox]{equation}
  f_\mathrm{env} \;:=\; \operatorname{ggT}\bigl\{\,f_n - f_m \;:\; n,m = 0,\dots,N-1\,\bigr\},
  \qquad
  T_\mathrm{env} := \frac{1}{f_\mathrm{env}} .
  \label{eq:fenvdef}
\end{empheq}
Das ist der größte gemeinsame Teiler aller Tonabstände \emph{einer} Achse.
Für äquidistante Töne nach Gl.~\eqref{eq:rf} ist jede Differenz ein
ganzzahliges Vielfaches des Nachbarabstands $d := \Delta f/(N-1)$, und $d$
selbst kommt als Differenz vor. Der ggT ist dann schlicht der Tonabstand:
\begin{equation}
  f_\mathrm{env} = d = \frac{\Delta f}{N-1}
  \qquad\text{(äquidistante Töne)} .
  \label{eq:fenvequi}
\end{equation}
Die allgemeine Definition Gl.~\eqref{eq:fenvdef} braucht man erst bei
ungleichmäßig verteilten Tönen; dann kann $f_\mathrm{env}$ deutlich kleiner
als jeder einzelne Abstand sein.

\paragraph{Warum genau diese Frequenz die Periode ist.}
Für den Betrag braucht man $|A|^2 = A A^*$. Mit Gl.~\eqref{eq:envdef}
...
\begin{equation}
  ...
  \label{eq:abs2expand}
\end{equation}
Zwei Dinge fallen auf. Erstens ist $f_\mathrm{c}$ verschwunden: Es kürzt sich
in jeder Differenz $(f_n-f_\mathrm{c})-(f_m-f_\mathrm{c})$. Zweitens kommen
nur noch die \emph{Differenzfrequenzen} $f_n-f_m$ vor, und nach
Gl.~\eqref{eq:fenvdef} ist jede davon ein ganzzahliges Vielfaches
$k\,f_\mathrm{env}$ mit $k\in\{-(N-1),\dots,N-1\}$. Gl.~\eqref{eq:abs2expand}
ist also eine endliche Fourierreihe mit Grundfrequenz $f_\mathrm{env}$,
\begin{equation}
  |A(t)|^2 = \sum_{k=-(N-1)}^{N-1} c_k\, e^{\,\mathrm{i}2\pi k f_\mathrm{env} t},
  \qquad
  c_k = \sum_{n-m=k} a_n a_m\, e^{\,\mathrm{i}(\varphi_n-\varphi_m)} ,
  \label{eq:abs2fourier}
\end{equation}
und daraus folgt sofort
\begin{equation}
  |A(t+T_\mathrm{env})| = |A(t)| \qquad\text{für alle } t \text{ und alle } \varphi_n .
  \label{eq:absperiodic}
\end{equation}
Die Phasen stecken nur in den Koeffizienten $c_k$, nicht in den Frequenzen.
Sie können die \emph{Form} der Einhüllenden ändern, aber nicht ihre Periode.

\paragraph{\boldmath$A$ selbst ist nur bis auf einen Faktor periodisch.}
Für $A(t)$ ohne Betrag gilt das nicht ganz. Nach Gl.~\eqref{eq:envdef} ist
$A(t+T_\mathrm{env}) = \sum_n a_n e^{\mathrm{i}[\dots]}\,e^{\,\mathrm{i}2\pi(f_n-f_\mathrm{c})T_\mathrm{env}}$,
und weil $f_n - f_\mathrm{off}$ ein Vielfaches von $f_\mathrm{env}$ ist, ist der
letzte Faktor für alle $n$ derselbe:
\begin{equation}
  A(t+T_\mathrm{env}) = e^{\,\mathrm{i}2\pi(f_\mathrm{off}-f_\mathrm{c})T_\mathrm{env}}\;A(t) .
\end{equation}
Ein Faktor vom Betrag eins, der von der frei gewählten Bezugsfrequenz
abhängt. Mit $f_\mathrm{c}=\bar f$ liegen die Töne der $x$-Achse bei
$\hat n f_\mathrm{env}$ mit $\hat n\in\{-1,0,1\}$, der Faktor ist $+1$.
Auf der $y$-Achse liegen sie bei $\hat n\in\{-\tfrac32,-\tfrac12,\tfrac12,\tfrac32\}$,
und der Faktor ist $e^{\mathrm{i}\pi}=-1$: $A$ wechselt nach einer Periode das
Vorzeichen, $|A|$ nicht. Numerisch an je 100 Zufallsphasensätzen geprüft:
$A(t+T_\mathrm{env})/A(t) = +1$ ($x$) bzw.\ $-1$ ($y$), und
$\max_t\bigl||A(t+T_\mathrm{env})|-|A(t)|\bigr| \le 5\cdot10^{-13}$. Deshalb
reden alle Aussagen dieses Textes über $|A|$, nie über $A$.

\paragraph{Das Signal selbst hat eine viel längere Periode.}
$f_\mathrm{env}$ ist \emph{nicht} die Periode von $s(t)$. Das volle Signal
wiederholt sich erst nach $1/\operatorname{ggT}\{f_n\}$, weil dafür auch der
Träger ganzzahlig viele Schwingungen machen muss. Am Arbeitspunkt ist
$\operatorname{ggT}\{100{,}000;\,100{,}185;\,100{,}370\}\,\si{\mega\hertz}
= \SI{5}{\kilo\hertz}$, also \SI{200}{\micro\second} auf der $x$-Achse, und
\SI{3.33}{\kilo\hertz}, also \SI{300}{\micro\second} auf der $y$-Achse.
Über so lange Zeiten muss man nie rechnen. Die Trennung aus Schritt~5 von
Abschnitt~1.1 macht die Trägerschwingung für $\max|s|$ und $\langle s^2\rangle$
bedeutungslos, und für $|A|$ reicht die kurze Periode $T_\mathrm{env}$.

\paragraph{Ist \boldmath$T_\mathrm{env}$ die \emph{kürzeste} Periode?}
Im Allgemeinen ja, aber nicht für jeden Phasensatz. Der Grundton der Reihe
Gl.~\eqref{eq:abs2fourier} ist
\begin{equation}
  c_1 = \sum_n a_{n+1}a_n\,e^{\,\mathrm{i}(\varphi_{n+1}-\varphi_n)} ,
\end{equation}
und der kann für besondere Phasen verschwinden. Beispiel $N=3$ mit
$\varphi=[0,\,90^\circ,\,0]$: $c_1 = \varrho\,(e^{\mathrm{i}\pi/2}+e^{-\mathrm{i}\pi/2}) = 0$,
und tatsächlich wird $|A|^2 = 4\varrho^2\cos^2\theta+1$ (Gl.~\eqref{eq:psiabs}
mit $\psi=90^\circ$), das schon nach $T_\mathrm{env}/2$ wiederkehrt. Für
das Mittelungsfenster schadet das nicht: $T_\mathrm{env}$ ist dann ein
ganzzahliges Vielfaches der kürzeren Periode, und ein Mittel über zwei
volle Perioden ist dasselbe wie über eine. \textbf{$T_\mathrm{env}$ ist die
kürzeste Fensterlänge, die für \emph{alle} Phasensätze gleichzeitig
richtig ist}, und genau das braucht eine Optimierung, die die Phasen
variiert. Jedes ganzzahlige Vielfache $k\,T_\mathrm{env}$ ist ebenfalls
richtig, jede andere Länge nicht.

\paragraph{Zahlen am Arbeitspunkt.}
\begin{center}
\begin{tabular}{@{}lcccccc@{}}
  \toprule
  & $N$ & $\Delta f$ & $f_\mathrm{env}=d$ & $T_\mathrm{env}$ & $1/\Delta f$ & Periode von $s(t)$\\
  \midrule
  $x$-Achse & 3 & \SI{370}{\kilo\hertz} & \SI{185.00}{\kilo\hertz} & \SI{5.405}{\micro\second} & \SI{2.703}{\micro\second} & \SI{200}{\micro\second}\\
  $y$-Achse & 4 & \SI{370}{\kilo\hertz} & \SI{123.33}{\kilo\hertz} & \SI{8.108}{\micro\second} & \SI{2.703}{\micro\second} & \SI{300}{\micro\second}\\
  \bottomrule
\end{tabular}
\end{center}
Die Spalte $1/\Delta f$ steht nur da, weil sie der naheliegende Fehler ist.
Die Spanne ist \emph{kein} Tonabstand, der in $|A|^2$ als Grundfrequenz
auftritt, sondern die größte Differenzfrequenz, die $(N-1)$-te Oberwelle.
Über $1/\Delta f$ kehrt $|A|$ nicht wieder: Um diese Zeit verschoben weicht
$|A|$ bei zufälligen Phasen um bis zu $2{,}0$ ($x$) bzw.\ $3{,}5$ ($y$) vom
Ausgangswert ab, bei einem Maximum von $2{,}97$ bzw.\ $4{,}15$.

\paragraph{Und die optische Schwebung?}
Die Spots entstehen aus Paaren je eines $x$- und $y$-Tons und liegen bei den
Summenfrequenzen $f_n+g_m$. Deren Differenzen sind
$k_x f_{\mathrm{env},x} + k_y f_{\mathrm{env},y}$, ihr ggT ist
\begin{equation}
  f_0 = \operatorname{ggT}\bigl(f_{\mathrm{env},x},\, f_{\mathrm{env},y}\bigr)
      = \operatorname{ggT}(185{,}00;\,123{,}33)\,\si{\kilo\hertz}
      = \SI{61.67}{\kilo\hertz},
  \qquad T_0 = \SI{16.216}{\micro\second} ,
\end{equation}
das kleinste gemeinsame Vielfache der beiden Achsenperioden
($3\cdot5{,}405 = 2\cdot8{,}108$). $f_\mathrm{env}$ gehört also zu
\emph{einer} HF-Kette und bestimmt deren Crestfaktor, $f_0$ gehört zum
Intensitätsmuster beider Achsen zusammen (Tabelle „Drei Perioden'' in
Abschnitt~\ref{sec:rmsphase}).
...
\paragraph{Stichprobe mit Zufallsphasen.}
Drei Phasensätze sind wenig. Deshalb zusätzlich je Achse 200 unabhängige,
gleichverteilte Phasensätze, Mittel über genau $T_\mathrm{env}$:
\begin{center}
\begin{tabular}{@{}lccc@{}}
  \toprule
  & $\sqrt{\sum a_n^2}$ & $\mathrm{rms}|A|$, 200 Sätze, $T=T_\mathrm{env}$ & $\mathrm{rms}|A|$, 200 Sätze, $T=1/\Delta f$\\
  \midrule
  $x$-Achse & $1{,}714642820$ & $1{,}714642820$, Spanne $4\cdot10^{-16}$ & $0{,}70 \dots 2{,}32$\\
  $y$-Achse & $2{,}078460969$ & $2{,}078460969$, Spanne $4\cdot10^{-16}$ & $0{,}76 \dots 3{,}32$\\
  \bottomrule
\end{tabular}
\end{center}
Die Streuung im richtigen Fenster ist Rundungsrauschen. Das gilt auch auf dem
diskreten Zeitraster, mit dem man in der Praxis rechnet: Bei $M$
gleichabständigen Stützstellen über $T_\mathrm{env}$ ist
$\frac1M\sum_j e^{\mathrm{i}2\pi k j/M}=0$ für jedes ganzzahlige $k$, das
kein Vielfaches von $M$ ist. Solange $M > N-1$, verschwinden die Mischterme
also auch in der Summe exakt, nicht nur im Integral.

\paragraph{Welche Voraussetzungen tatsächlich eingehen.}
Die Phasenunabhängigkeit braucht genau zwei Dinge. Erstens verschiedene
Tonfrequenzen: Fielen zwei Töne zusammen, wäre ihr Mischterm eine Konstante
und das Ergebnis $|a_n e^{\mathrm{i}\varphi_n}+a_m e^{\mathrm{i}\varphi_m}|^2$
hinge sehr wohl von $\varphi_n-\varphi_m$ ab. Zweitens das Fenster
$k\,T_\mathrm{env}$ nach Abschnitt~\ref{sec:fenv}. Die Amplituden dürfen
beliebig sein, die Zahl der Töne auch. Die einzige Näherung, die Trennung
von Träger und Einhüllender in $\langle s^2\rangle = \tfrac12\langle|A|^2\rangle$,
hat mit den Phasen nichts zu tun und ist auf $10^{-4}$ genau.
...
gemeinsamen Teiler der Tonabstände dieser Achse (Abschnitt~\ref{sec:fenv}) -- nicht über $1/\Delta f$
...
und verträgt die Kette $P_\mathrm{peak}\le P_\mathrm{max}$, so folgt
$P_\mathrm{mittel}\le P_\mathrm{max}/\crest^{2}$. Die Systemimpedanz $R$
...
Minimalwert $2{,}196$ wird bei $b=98^\circ$ erreicht. Dass das kein Zufall der Stichprobe ist, sondern exakt das Optimum der Familie, zeigt Abschnitt~\ref{sec:n4} in geschlossener Form.
...
\subsection{Wovon das Maximum abhängt -- und \boldmath$N=4$ in geschlossener Form}
\label{sec:n4}

Der Nenner des Crestfaktors ist nach Abschnitt~\ref{sec:rmsphase} fest.
Bleibt zu klären, von welchen Phasenkombinationen der Zähler $\max_t|A|$
überhaupt abhängt, und warum auf der $y$-Achse gerade $b=98^\circ$ das
Optimum ist.

\paragraph{Zwei Operationen ändern das Maximum nie.}
\begin{enumerate}\itemsep0.15em
  \item Eine \emph{globale} Phase, $\varphi_n\to\varphi_n+\alpha$:
        $A\to e^{\mathrm{i}\alpha}A$, $|A|$ unverändert.
  \item Eine \emph{Rampe}, $\varphi_n\to\varphi_n+\delta\,n$: nach
        Abschnitt~\ref{sec:rampen} eine Zeitverschiebung
        $A(t)\to A(t+\tau)$ bis auf einen globalen Faktor. Das Maximum
        über eine volle Periode ändert sich dadurch nicht, es wird nur
        früher oder später erreicht.
\end{enumerate}
Numerisch: Auf beide Arbeitspunktsätze zufällige $\alpha$ und $\delta$
aufgeschlagen, $\max|A|$ bleibt $2{,}96977156$ bzw.\ $3{,}22785026$ auf allen
acht Stellen.

\paragraph{Was übrig bleibt: die Krümmungen.}
Von $N$ Phasen verbrauchen diese beiden Operationen zwei Freiheitsgrade.
Übrig bleiben $N-2$ Kombinationen, die unter beiden invariant sind. Eine
bequeme Wahl sind die \emph{zweiten Differenzen}
\begin{equation}
  \beta_k \;:=\; \varphi_k - 2\varphi_{k+1} + \varphi_{k+2},
  \qquad k = 0,\dots,N-3 .
  \label{eq:betadef}
\end{equation}
Eine Konstante und ein linearer Term in $n$ fallen in einer zweiten Differenz
heraus, und umgekehrt legen die $\beta_k$ zusammen mit $\varphi_0$ und
$\varphi_1$ alle Phasen fest. Also gilt
\begin{equation}
  \max_t|A| = F(\beta_0,\dots,\beta_{N-3})
  \qquad\text{und sonst von nichts.}
\end{equation}
Für $N=3$ gibt es genau eine, $\beta_0 = -2\psi$ mit $\psi$ aus
Gl.~\eqref{eq:psidef}; das ist die Krümmung aus
Abschnitt~\ref{sec:psi}. Für $N=4$ gibt es zwei.

\paragraph{Was die Zentrierung daraus macht.}
Mit dem Zentrierungskriterium $\varphi_n+\varphi_{N-1-n}=c$ ist
\begin{align}
  N=3:&\quad \beta_0 = (\varphi_0+\varphi_2) - 2\varphi_1 = c - 2\varphi_1
        \in \{0,\,2\pi\} \equiv 0
        &&\Rightarrow\ \text{kein Parameter},\\
  N=4:&\quad \beta_0 = \varphi_0-2\varphi_1+\varphi_2,\quad
        \beta_1 = \varphi_1-2\varphi_2+\varphi_3 = -\beta_0
        &&\Rightarrow\ \text{ein Parameter}.
\end{align}
Die zweite Zeile folgt, indem man $\varphi_3=c-\varphi_0$ und
$\varphi_2=c-\varphi_1$ einsetzt. Mit der Schreibweise
$\varphi_y=[\,0,\,b,\,c-b,\,c\,]$ aus Abschnitt~\ref{sec:crestback} ist
\begin{equation}
  \beta_0 = c - 3b, \qquad \beta_1 = 3b - c .
\end{equation}
Die Rampe $b=c/3$ ist $\beta_0=0$. Am Arbeitspunkt, $c=114^\circ$ und
$b=98^\circ$, ist $\beta_0 = 114^\circ-294^\circ = -180^\circ$ und
$\beta_1=+180^\circ$: \textbf{genau der Punkt, der von der Rampe am weitesten
entfernt ist.}

\paragraph{Die Einhüllende der zentrierten Familie ist reell -- Schritt für Schritt.}
Wie bei $N=3$ rechnet man direkt. Weil hier mehr passiert als dort, in
kleinen Schritten.

\smallskip
\noindent\textit{Schritt a: Wo die Töne relativ zu \boldmath$f_\mathrm{c}$ liegen.}
Die Töne sind $f_n = f_\mathrm{off} + n\,f_\mathrm{env}$, $n=0,1,2,3$. Ihr
Mittelwert ist
$\bar f = f_\mathrm{off} + \tfrac{0+1+2+3}{4}\,f_\mathrm{env} = f_\mathrm{off}+\tfrac32 f_\mathrm{env}$.
Mit $f_\mathrm{c}=\bar f$ (erlaubt, weil $f_\mathrm{c}$ beliebig ist,
Abschnitt~1) ist
\begin{equation}
  f_n - f_\mathrm{c} = \bigl(n-\tfrac32\bigr) f_\mathrm{env} =: \hat n\, f_\mathrm{env},
  \qquad \hat n \in \bigl\{-\tfrac32,\,-\tfrac12,\,+\tfrac12,\,+\tfrac32\bigr\} .
\end{equation}
Die Töne liegen jetzt symmetrisch um null: Ton $0$ und Ton $3$ bei
$\mp\tfrac32 f_\mathrm{env}$ (das \emph{äußere} Paar), Ton $1$ und Ton $2$ bei
$\mp\tfrac12 f_\mathrm{env}$ (das \emph{innere} Paar).

\smallskip
\noindent\textit{Schritt b: Eine bequeme Zeitvariable.}
Ton $n$ trägt in $A(t)$ den Faktor
$e^{\mathrm{i}(2\pi\hat n f_\mathrm{env}t + \varphi_n)}$. Die halbzahligen
$\hat n$ sind lästig; mit
\begin{equation}
  u := \pi f_\mathrm{env}\,t
  \qquad\Longrightarrow\qquad
  2\pi\hat n f_\mathrm{env} t = 2\hat n\,u \in \{-3u,\,-u,\,+u,\,+3u\}
\end{equation}
werden daraus ganze Vielfache. Eine Periode $t\in[0,T_\mathrm{env})$
entspricht $u\in[0,\pi)$.

\smallskip
\noindent\textit{Schritt c: Die Phasen nach der Zentrierung zerlegen.}
Das Zentrierungskriterium verlangt $\varphi_n+\varphi_{3-n}=c$. Man zieht
von jeder Phase die Hälfte davon ab,
\begin{equation}
  \chi_n := \varphi_n - \tfrac{c}{2}
  \qquad\Longrightarrow\qquad
  \chi_n + \chi_{3-n} = \varphi_n+\varphi_{3-n} - c = 0,
  \qquad\text{also}\qquad \chi_{3-n} = -\chi_n .
\end{equation}
Die $\chi$ gespiegelter Töne sind entgegengesetzt gleich. Frei bleiben zwei
Werte, einer pro Paar; man nennt sie
\begin{equation}
  \chi_\mathrm{a} := \chi_3 = -\chi_0 \quad\text{(außen)},
  \qquad
  \chi_\mathrm{i} := \chi_2 = -\chi_1 \quad\text{(innen)} .
\end{equation}
Mit $\varphi = [\,0,\,b,\,c-b,\,c\,]$ und am Arbeitspunkt ($c=114^\circ$,
$b=98^\circ$):
\begin{center}
\begin{tabular}{@{}cccccc@{}}
  \toprule
  $n$ & $\hat n$ & $a_n$ & $\varphi_n$ & $\chi_n=\varphi_n-c/2$ allgemein & am Arbeitspunkt\\
  \midrule
...
\smallskip
\noindent\textit{Schritt d: Einsetzen und \boldmath$e^{\mathrm{i}c/2}$ ausklammern.}
Jeder Term ist
$a_n e^{\mathrm{i}(2\hat n u+\varphi_n)} = e^{\mathrm{i}c/2}\cdot a_n e^{\mathrm{i}(2\hat n u+\chi_n)}$,
der Faktor $e^{\mathrm{i}c/2}$ ist für alle vier gleich. Die vier Terme
nach Division durch ihn, mit $\hat n$ und $\chi_n$ aus der Tabelle:
\begin{align}
  A(t)\,e^{-\mathrm{i}c/2}
  &= \underbrace{\varrho\,e^{\mathrm{i}(-3u-\chi_\mathrm{a})}}_{n=0}
   + \underbrace{e^{\mathrm{i}(-u-\chi_\mathrm{i})}}_{n=1}
   + \underbrace{e^{\mathrm{i}(+u+\chi_\mathrm{i})}}_{n=2}
   + \underbrace{\varrho\,e^{\mathrm{i}(+3u+\chi_\mathrm{a})}}_{n=3} .
\end{align}

\smallskip
\noindent\textit{Schritt e: Paarweise zusammenfassen.}
Term $0$ und Term $3$ haben denselben Betrag $\varrho$ und entgegengesetzte
Exponenten, ebenso Term $1$ und Term $2$. Mit
$e^{\mathrm{i}x}+e^{-\mathrm{i}x} = 2\cos x$:
\begin{align}
...
\begin{equation}
  ...
  \label{eq:n4pair}
\end{equation}
Das ist eine \emph{reelle} Zahl. Genau das leistet die Zentrierung: Sie
macht gespiegelte Zeiger zu konjugierten Paaren, deren Summe keinen
Imaginärteil hat. Der Betrag ist damit schlicht
$|A| = |2\varrho\cos(3u+\chi_\mathrm{a}) + 2\cos(u+\chi_\mathrm{i})|$; am
Arbeitspunkt $|A| = |2\varrho\cos(3u+57^\circ) + 2\cos(u-41^\circ)|$.

\smallskip
\noindent\textit{Schritt f: Den Zeitnullpunkt verschieben.}
Ein anderer Zeitnullpunkt ändert das Maximum über eine volle Periode nicht,
er ändert nur, \emph{wann} es erreicht wird. Man wählt ihn so, dass das
innere Paar keine Phase mehr hat: neue Variable $u' := u + \chi_\mathrm{i}$,
also $u = u'-\chi_\mathrm{i}$. Dann
\begin{align}
  u+\chi_\mathrm{i} &= u',\\
  3u+\chi_\mathrm{a} &= 3u' - 3\chi_\mathrm{i} + \chi_\mathrm{a} = 3u' + \gamma,
  \qquad \gamma := \chi_\mathrm{a}-3\chi_\mathrm{i} .
\end{align}
Nennt man $u'$ wieder $u$:
\begin{empheq}[box=\fbox]{equation}
  A\,e^{-\mathrm{i}c/2} \;=\; 2\varrho\cos(3u+\gamma) + 2\cos u,
  \qquad
  \gamma = \chi_\mathrm{a}-3\chi_\mathrm{i}
         = \tfrac{c}{2} - 3\bigl(\tfrac{c}{2}-b\bigr) = 3b-c = -\beta_0 .
  \label{eq:n4env}
\end{empheq}
Am Arbeitspunkt: $\gamma = 57^\circ - 3\cdot(-41^\circ) = 57^\circ+123^\circ = 180^\circ$,
oder ebenso $3\cdot98^\circ-114^\circ = 180^\circ$.

Die Periode bleibt richtig: Mit $u\to u+\pi$ wechseln $\cos u$ und
$\cos(3u+\gamma)$ beide das Vorzeichen, der Ausdruck also auch, der Betrag
nicht -- $|A|$ hat in $u$ die Periode $\pi$, wie es nach Schritt~b sein muss.

\smallskip
\noindent\textit{Schritt g: Warum bei \boldmath$N=4$ ein Parameter überlebt.}
Eine Zeitverschiebung $\Delta t$ dreht jeden Ton um $2\pi(f_n-f_\mathrm{c})\Delta t$,
also proportional zu seiner Frequenz: das innere Paar um $\Delta u$, das
äußere um $3\Delta u$. Man kann damit $\chi_\mathrm{i}$ auf null schieben,
aber $\chi_\mathrm{a}$ wandert dabei um das Dreifache mit. Was sich nicht
wegschieben lässt, ist genau die Kombination $\chi_\mathrm{a}-3\chi_\mathrm{i}=\gamma$.
Gegenprobe mit einer Rampe $\varphi_n\to\varphi_n+\delta n$: Dann wird
$c\to c+3\delta$ und $\chi_n\to\chi_n+\delta\hat n$, also
$\chi_\mathrm{a}\to\chi_\mathrm{a}+\tfrac32\delta$ und
$\chi_\mathrm{i}\to\chi_\mathrm{i}+\tfrac12\delta$, und
$\gamma\to\gamma+\tfrac32\delta-3\cdot\tfrac12\delta=\gamma$ -- unverändert,
wie es für eine Zeitverschiebung sein muss. Bei $N=3$ gab es nur ein Paar
($\hat n=\pm1$) und einen mittleren Ton bei $\hat n=0$, den keine
Zeitverschiebung dreht; das eine $\chi$ ließ sich vollständig wegschieben.

\paragraph{Die beiden Grenzfälle.}
\emph{Rampe}, $\gamma=0$: $A\,e^{-\mathrm{i}c/2} = 2\varrho\cos3u + 2\cos u$.
Bei $u=0$ stehen beide Kosinus auf $1$, und größer als $2\varrho+2$ kann der
Betrag nach der Dreiecksungleichung nie werden:
\begin{equation}
  \max|A| = 2\varrho+2 = \textstyle\sum_n a_n = 4{,}1540659,
  \qquad \crest_y = \sqrt2\,\frac{4{,}1540659}{2{,}0784610} = 2{,}8264839 .
\end{equation}

\emph{Arbeitspunkt}, $\gamma=\pi$: Mit $\cos(3u+\pi) = -\cos3u$ wird
\begin{equation}
  A\,e^{-\mathrm{i}c/2} = 2\cos u - 2\varrho\cos 3u .
\end{equation}
Bei $u=0$ stehen die beiden Paare jetzt \emph{gegeneinander}: $2-2\varrho$,
fast null. Wo liegt das Maximum?

\smallskip
\noindent\textit{Schritt h: Auf ein Polynom zurückführen.}
Die Dreifachwinkelformel folgt aus dem Additionstheorem,
$\cos3u = \cos(2u+u) = \cos2u\cos u - \sin2u\sin u
= (2\cos^2u-1)\cos u - 2\sin^2u\cos u = 4\cos^3u - 3\cos u$.
Mit $v := \cos u$:
\begin{equation}
  G(v) := A\,e^{-\mathrm{i}c/2}
  = 2v - 2\varrho\,(4v^3-3v)
  = (2+6\varrho)\,v - 8\varrho\,v^3 .
\end{equation}
Während $u$ eine Periode $[0,\pi)$ durchläuft, nimmt $v=\cos u$ jeden Wert in
$[-1,1]$ an. Das Maximum von $|A|$ über die Zeit ist also das Maximum von
$|G(v)|$ über $v\in[-1,1]$.

\smallskip
\noindent\textit{Schritt i: Nur \boldmath$v\ge0$ ansehen.}
$G(-v) = -G(v)$, $G$ ist ungerade; $|G|$ ist deshalb symmetrisch, und es
reicht $v\in[0,1]$. Kandidaten für das Maximum sind die Ränder und die
...
\smallskip
\noindent\textit{Schritt j: Die Extremstelle.}
$G'(v_*)=0$:
\begin{equation}
  v_*^2 = \frac{2+6\varrho}{24\varrho} = \frac{1+3\varrho}{12\varrho} .
\end{equation}
Wegen $G''(v) = -48\varrho v < 0$ für $v>0$ ist das ein Maximum. Es liegt
im Intervall, solange $v_*\le1$, also $1+3\varrho\le12\varrho$, also
$\varrho\ge1/9$ -- für jede sinnvolle Amplitude erfüllt.

\smallskip
\noindent\textit{Schritt k: Der Wert.}
Man setzt $v_*^2$ in die Klammer von $G(v_*) = v_*\bigl[(2+6\varrho) - 8\varrho v_*^2\bigr]$ ein:
\begin{equation}
...
Also
\begin{equation}
  G(v_*) = \tfrac43(1+3\varrho)\cdot v_*
...
\end{equation}
mit $\sqrt{12\varrho} = 2\sqrt{3\varrho}$ im vorletzten Schritt. Der Rand
$|G(1)| = |2-2\varrho|$ ist für $\varrho$ um $1$ viel kleiner, also
\begin{empheq}[box=\fbox]{equation}
  \max_t|A| = \frac{2}{3}\,\frac{(1+3\varrho)^{3/2}}{\sqrt{3\varrho}},
  \qquad
  \crest_y = \sqrt2\,\frac{\tfrac23(1+3\varrho)^{3/2}/\sqrt{3\varrho}}{\sqrt{2\varrho^2+2}} .
  \label{eq:n4closed}
\end{empheq}

\smallskip
\noindent\textit{Schritt l: Zahlen.} Mit $\varrho=\sqrt{1{,}16}=1{,}0770330$:
\begin{center}
\begin{tabular}{@{}lr@{}}
  \toprule
  $1+3\varrho$ & $4{,}2310991$\\
  $12\varrho$ & $12{,}9243955$\\
  $v_*^2 = 4{,}2310991/12{,}9243955$ & $0{,}3273731$\\
  $v_* = \cos u_*$ & $0{,}5721652$ \quad($u_*=55{,}10^\circ$ in der verschobenen Zeit)\\
  $\tfrac43(1+3\varrho)$ & $5{,}6414655$\\
  $\max|A| = 5{,}6414655\cdot0{,}5721652$ & $3{,}2278503$\\
  Randwert $|G(1)| = |2-2\varrho|$ & $0{,}1540659$\\
  $\sqrt{\sum a_n^2} = \sqrt{2r_y+2} = \sqrt{4{,}32}$ & $2{,}0784610$\\
  $\crest_y = \sqrt2\cdot3{,}2278503/2{,}0784610$ & $\mathbf{2{,}1962739}$\\
  \bottomrule
\end{tabular}
\end{center}

\smallskip
\noindent\textit{Schritt m: Zurück in die echte Zeit.}
Das Maximum liegt bei $v=+v_*$, also $u'=55{,}10^\circ$, und wegen
$|G(-v)|=|G(v)|$ ebenso bei $v=-v_*$, $u'=180^\circ-55{,}10^\circ=124{,}90^\circ$.
Mit $u = u'-\chi_\mathrm{i} = u'+41^\circ$ und $t = u/(\pi f_\mathrm{env}) = (u/180^\circ)\,T_\mathrm{env}$:
\begin{equation}
  u = 96{,}10^\circ \;\Rightarrow\; t = \SI{4.3288}{\micro\second},
  \qquad
  u = 165{,}90^\circ \;\Rightarrow\; t = \SI{7.4730}{\micro\second},
\end{equation}
genau die beiden Zeitpunkte aus Abschnitt~\ref{sec:handcalc} und
Abschnitt~\ref{sec:ycrest}.

Das ist auf acht Stellen der numerisch gefundene Wert aus
Abschnitt~\ref{sec:crestback}, jetzt aber mit Begründung. Zum Vergleich bei
gleichen Amplituden, $\varrho=1$: $\max|A| = \tfrac23\cdot8/\sqrt3 = 3{,}0792$,
$\crest_y = 2{,}1773$.

\paragraph{Dazwischen: monoton.}
Für allgemeines $\gamma$ gibt es keine so kurze Formel, numerisch ist der
Verlauf aber eindeutig ($c=114^\circ$, $b = (c+\gamma)/3$):
\begin{center}
\begin{tabular}{@{}lccccccc@{}}
  \toprule
  $|\gamma| = |\beta_0|$ & $0^\circ$ & $30^\circ$ & $60^\circ$ & $90^\circ$ & $120^\circ$ & $150^\circ$ & $180^\circ$\\
  $b$ für $\gamma\ge0$ & $38^\circ$ & $48^\circ$ & $58^\circ$ & $68^\circ$ & $78^\circ$ & $88^\circ$ & $98^\circ$\\
  \midrule
  $\crest_y$ & $2{,}8265$ & $2{,}8077$ & $2{,}7519$ & $2{,}6603$ & $2{,}5350$ & $2{,}3791$ & $\mathbf{2{,}1963}$\\
  \bottomrule
\end{tabular}
\end{center}
$\crest_y$ hängt nur von $|\gamma|$ ab und fällt monoton von der Rampe bis
$\gamma=\pi$. Die Werte der Tabelle in Abschnitt~\ref{sec:crestback} liegen
auf dieser Kurve: $b=60^\circ$ ist $\gamma=66^\circ$ ($2{,}7364$),
$b=114^\circ$ ist $\gamma=228^\circ\equiv-132^\circ$ ($2{,}4761$).
\textbf{Innerhalb der zentrierten Familie ist $b=98^\circ$ also exakt das
Optimum, nicht nur der beste der probierten Werte.}

\paragraph{Ohne Zentrierung.}
Lässt man $\beta_0$ und $\beta_1$ unabhängig laufen, liegt das Minimum bei
$\beta_0=\beta_1=-102{,}76^\circ$ mit $\crest_y = 1{,}766130$ (Gitter in
$3^\circ$-Schritten, dann Nelder--Mead). Dieser Punkt verletzt
$\beta_1=-\beta_0$ und ist deshalb für den Aufbau nicht zulässig. Die
Differenz $2{,}1963 \to 1{,}7661$ ist der Preis der Zentrierung auf der
$y$-Achse.
...
\begin{tabular}{@{}lccc@{}}
  \toprule
  & wirksame Parameter & mit Zentrierung & $\crest$ am Arbeitspunkt\\
  \midrule
  $x$, $N=3$ & $\beta_0=-2\psi$ & $0$ & $2{,}4494263 = $ Maximum\\
  $y$, $N=4$ & $\beta_0,\ \beta_1$ & $1$ ($\gamma=-\beta_0$) & $2{,}1962739 = $ Familienminimum\\
  \bottomrule
\end{tabular}
\end{center}

\subsection{Nachrechnung am Arbeitspunkt: \boldmath$\max|A|$ Schritt für Schritt}
\label{sec:handcalc}

Die beiden vorigen Abschnitte rechnen mit der ganzen Familie. Hier dasselbe
direkt mit den eingestellten Phasen, so dass jeder Schritt mit dem
Taschenrechner nachvollziehbar ist. Zusätzlich sind Zeitpunkt und
Zeigerstellung des Maximums angegeben; beides lässt sich numerisch
überprüfen.

\paragraph{\boldmath$x$-Achse, $\varphi_x=[0;\,12{,}5;\,25]^\circ$.}
Bezugsfrequenz ist der mittlere Ton, $\theta := 2\pi\cdot\SI{185}{\kilo\hertz}\cdot t$,
$\varrho=\sqrt{0{,}97}=0{,}9848858$:
\begin{align}
  A(t) &= \varrho\,e^{\mathrm{i}(0^\circ-\theta)} + e^{\mathrm{i}\,12{,}5^\circ}
          + \varrho\,e^{\mathrm{i}(25^\circ+\theta)} ,\\
  A(t)\,e^{-\mathrm{i}\,12{,}5^\circ}
       &= \varrho\,e^{-\mathrm{i}(\theta+12{,}5^\circ)} + 1
          + \varrho\,e^{+\mathrm{i}(\theta+12{,}5^\circ)}
        = 2\varrho\cos(\theta+12{,}5^\circ) + 1 .
\end{align}
Das Maximum liegt bei $\cos(\theta+12{,}5^\circ)=1$, also $\theta=-12{,}5^\circ$
oder, in die Periode $T_\mathrm{env}=\SI{5.4054}{\micro\second}$ gelegt,
\begin{equation}
  t_\mathrm{max} = \Bigl(1-\frac{12{,}5}{360}\Bigr)\,T_\mathrm{env} = \SI{5.2177}{\micro\second} .
\end{equation}
Zu diesem Zeitpunkt haben die drei Zeiger die Winkel
$0^\circ+12{,}5^\circ$, $12{,}5^\circ$ und $25^\circ-12{,}5^\circ$ -- alle
$12{,}5^\circ$. Sie liegen exakt auf einer Linie, und
\begin{equation}
  \max|A| = 2\varrho+1 = 2\cdot0{,}9848858+1 = 2{,}9697716,
  \qquad
  \crest_x = \sqrt2\,\frac{2{,}9697716}{\sqrt{2\cdot0{,}97+1}}
           = \sqrt2\,\frac{2{,}9697716}{1{,}7146428} = 2{,}4494263 .
\end{equation}
Die Rampe $12{,}5^\circ$ pro Ton verschiebt nur den Zeitpunkt, an dem das
passiert (Abschnitt~\ref{sec:rampen}).

\paragraph{\boldmath$y$-Achse, $\varphi_y=[0;\,98;\,16;\,114]^\circ$.}
Bezugsfrequenz ist der Mittelwert der vier Töne; die Töne liegen dann bei
$\hat n\cdot\SI{123.33}{\kilo\hertz}$ mit $\hat n=-\tfrac32,-\tfrac12,\tfrac12,\tfrac32$.
Mit $u := \pi\cdot\SI{123.33}{\kilo\hertz}\cdot t$ hat Ton $n$ die Phase
$\varphi_n + 2\hat n\,u$. Zieht man $e^{\mathrm{i}\,57^\circ}$ heraus
($57^\circ = c_y/2$), stehen die vier Zeiger bei
\begin{equation}
  -57^\circ-3u,\qquad 41^\circ-u,\qquad -41^\circ+u,\qquad 57^\circ+3u .
\end{equation}
Erster und vierter sowie zweiter und dritter sind zueinander konjugiert --
das ist das Zentrierungskriterium --, die Summe ist deshalb reell:
\begin{equation}
  A\,e^{-\mathrm{i}\,57^\circ} = 2\varrho\cos(3u+57^\circ) + 2\cos(u-41^\circ) .
\end{equation}
Mit $w := u-41^\circ$ wird $3u+57^\circ = 3w+180^\circ$, also
\begin{equation}
  A\,e^{-\mathrm{i}\,57^\circ} = 2\cos w - 2\varrho\cos 3w .
\end{equation}
Das ist Gl.~\eqref{eq:n4env} mit $\gamma=\pi$, jetzt direkt aus den
eingestellten Phasen. Mit $v:=\cos w$ und $\cos3w=4v^3-3v$ folgt
$G(v)=(2+6\varrho)\,v-8\varrho\,v^3$. Die Zahlen mit
$\varrho=\sqrt{1{,}16}=1{,}0770330$:
\begin{center}
\begin{tabular}{@{}lr@{}}
  \toprule
  $2+6\varrho$ & $8{,}462198$\\
  $8\varrho$ & $8{,}616264$\\
  $v_*^2 = (1+3\varrho)/(12\varrho) = 4{,}231099/12{,}924396$ & $0{,}327372$\\
  $v_* = \cos w_*$ & $0{,}572165$ \quad ($w_*=55{,}10^\circ$)\\
  $G(v_*) = 0{,}572165\cdot(8{,}462198-8{,}616264\cdot0{,}327372)$ & $\mathbf{3{,}227850}$\\
  Randwert $|G(1)| = |2-2\varrho|$ & $0{,}154$\\
  \bottomrule
\end{tabular}
\end{center}
Der Randwert ist viel kleiner, das Maximum liegt also im Inneren.

\paragraph{Wie die Zeiger im Maximum stehen.}
$w_*=55{,}10^\circ$ heißt $u_*=96{,}10^\circ$ und damit
$t_\mathrm{max} = u_*/(\pi\cdot\SI{123.33}{\kilo\hertz}) = \SI{4.3288}{\micro\second}$
in der Periode $T_\mathrm{env}=\SI{8.1081}{\micro\second}$. Eingesetzt in die
vier Winkel oben:
\begin{equation}
  -57^\circ-288{,}3^\circ \equiv +14{,}7^\circ,\quad
  41^\circ-96{,}1^\circ = -55{,}1^\circ,\quad
  -41^\circ+96{,}1^\circ = +55{,}1^\circ,\quad
  57^\circ+288{,}3^\circ \equiv -14{,}7^\circ .
\end{equation}
Die Projektion auf die reelle Achse ist
\begin{equation}
  \max|A| = 2\varrho\cos14{,}7^\circ + 2\cos55{,}1^\circ
          = 2{,}08355 + 1{,}14433 = 3{,}2278503 ,
\end{equation}
und
\begin{equation}
  \crest_y = \sqrt2\,\frac{3{,}2278503}{\sqrt{2\cdot1{,}16+2}}
           = \sqrt2\,\frac{3{,}2278503}{2{,}0784610} = 2{,}1962739 .
\end{equation}
Das ist der Unterschied zur $x$-Achse in einem Satz: Dort gibt es einen
Zeitpunkt, an dem alle Zeiger auf einer Linie liegen. Hier kommt dieser
Zeitpunkt nie. Im besten Moment steht das äußere Paar bei $\pm14{,}7^\circ$
und das innere bei $\pm55{,}1^\circ$, und $\max|A|$ bleibt um
\SI{22}{\percent} unter $\sum_n a_n = 4{,}154$.

\paragraph{Numerische Gegenprobe.}
$|A(t)|$ direkt aus den realen Tonfrequenzen auf $2\cdot10^6$ Stützstellen
über eine Periode $T_\mathrm{env}$ ausgewertet, ohne jede der obigen
Umformungen:
\begin{center}
\begin{tabular}{@{}lcccc@{}}
  \toprule
  & $\max|A|$ & $t_\mathrm{max}$ & Zeigerwinkel relativ zur Summe & $\crest$\\
  \midrule
  $x$ & $2{,}9697716$ & \SI{5.2177}{\micro\second} & $[0;\,0;\,0]^\circ$ & $2{,}4494263$\\
  $y$ & $3{,}2278503$ & \SI{4.3288}{\micro\second} & $[+14{,}7;\,-55{,}1;\,+55{,}1;\,-14{,}7]^\circ$ & $2{,}1962739$\\
  \bottomrule
\end{tabular}
\end{center}
Alle Werte stimmen auf sieben Stellen mit der Handrechnung überein. Das
Skript dazu:
\begin{verbatim}
import numpy as np
def check(f_Hz, r, phi_deg):
    f = np.array(f_Hz, float); a = np.sqrt(r); phi = np.radians(phi_deg)
    T = 1/(f[1]-f[0])                                   # T_env
    t = np.linspace(0, T, 2_000_001)
    A = (a[:,None]*np.exp(1j*(2*np.pi*(f-f.mean())[:,None]*t
                              + phi[:,None]))).sum(0)
    i = np.argmax(abs(A))
    C = np.sqrt(2)*abs(A[i])/np.sqrt((a**2).sum())
    print(f"max|A| = {abs(A[i]):.7f}  t = {t[i]*1e6:.4f} us  C = {C:.7f}")
check([100.000e6, 100.185e6, 100.370e6], [0.97, 1, 0.97], [0, 12.5, 25])
check([100e6 + k*370e3/3 for k in range(4)], [1.16, 1, 1, 1.16],
      [0, 98, 16, 114])
\end{verbatim}

\subsection{Der Crestfaktor der \boldmath$y$-Achse vollständig nachgerechnet}
\label{sec:ycrest}

Abschnitt~\ref{sec:handcalc} rechnet den Zähler $\max|A|$. Hier die ganze
Kette für die $y$-Achse mit der eingestellten Phasenwahl, vom Eingang bis zum
Crestfaktor, mit jedem Zwischenwert.

\paragraph{Schritt 1: Eingangsgrößen.}
\begin{center}
\begin{tabular}{@{}lll@{}}
  \toprule
  Töne & $f_m = \SI{100}{\mega\hertz} + m\,f_\mathrm{env}$, $m=0\dots3$ & $f_\mathrm{env} = \SI{123.33}{\kilo\hertz}$, $T_\mathrm{env}=\SI{8.1081}{\micro\second}$\\
  Leistungsgewichte & $[\,1{,}16;\;1;\;1;\;1{,}16\,]$ & $r_y = 1{,}16$\\
  Spannungsamplituden & $a = [\,\varrho;\;1;\;1;\;\varrho\,]$ & $\varrho = \sqrt{1{,}16} = 1{,}0770330$\\
  Phasen & $\varphi = [\,0;\;98;\;16;\;114\,]^\circ$ & \\
  \bottomrule
\end{tabular}
\end{center}

\paragraph{Schritt 2: Die Phasenwahl einordnen.}
Paarsummen $\varphi_0+\varphi_3 = 0^\circ+114^\circ = 114^\circ$ und
$\varphi_1+\varphi_2 = 98^\circ+16^\circ = 114^\circ$: Das Zentrierungskriterium
ist mit $c_y = 114^\circ$ erfüllt, und in der Schreibweise
$[\,0,\,b,\,c_y-b,\,c_y\,]$ ist $b=98^\circ$. Der einzige crestwirksame
Parameter ist nach Gl.~\eqref{eq:n4env}
\begin{equation}
  \gamma = 3b - c_y = 294^\circ - 114^\circ = 180^\circ ,
\end{equation}
der Wert, bei dem die Familie ihr Minimum hat (Abschnitt~\ref{sec:n4}). Der
Crestfaktor sieht \emph{nur} $\gamma$. $c_y$ und eine zusätzliche Rampe
bleiben frei für die Optik; zu jedem $c_y$ gehört dasselbe Optimum bei
$b = (c_y+180^\circ)/3$.

\paragraph{Schritt 3: \boldmath$|A(t)|^2$ ausmultipliziert.}
Nach Gl.~\eqref{eq:abs2fourier} ist $|A|^2 = c_0 + 2\,\mathrm{Re}\sum_{k=1}^{3} c_k\,e^{\mathrm{i}k\theta}$
mit $\theta := 2\pi f_\mathrm{env} t$ und $c_k = \sum_{n-m=k} a_n a_m e^{\mathrm{i}(\varphi_n-\varphi_m)}$.
Die sechs Mischterme:
\begin{center}
\begin{tabular}{@{}cccccc@{}}
  \toprule
  $(n,m)$ & $k=n-m$ & $f_n-f_m$ & $a_na_m$ & $\varphi_n-\varphi_m$ & gehört zu\\
  \midrule
  $(1,0)$ & 1 & \SI{123.3}{\kilo\hertz} & $\varrho = 1{,}0770330$ & $+98^\circ$ & $c_1$\\
  $(2,1)$ & 1 & \SI{123.3}{\kilo\hertz} & $1$ & $-82^\circ = 98^\circ-180^\circ$ & $c_1$\\
  $(3,2)$ & 1 & \SI{123.3}{\kilo\hertz} & $\varrho = 1{,}0770330$ & $+98^\circ$ & $c_1$\\
  $(2,0)$ & 2 & \SI{246.7}{\kilo\hertz} & $\varrho = 1{,}0770330$ & $+16^\circ$ & $c_2$\\
  $(3,1)$ & 2 & \SI{246.7}{\kilo\hertz} & $\varrho = 1{,}0770330$ & $+16^\circ$ & $c_2$\\
  $(3,0)$ & 3 & \SI{370.0}{\kilo\hertz} & $\varrho^2 = 1{,}16$ & $+114^\circ$ & $c_3$\\
  \bottomrule
\end{tabular}
\end{center}
Innerhalb jedes $c_k$ haben alle Terme dieselbe Richtung, bis auf das
Vorzeichen des mittleren Paares in $c_1$ -- das ist das $\gamma=180^\circ$.
Deshalb lassen sich die Koeffizienten geschlossen angeben:
\begin{align}
  c_0 &= \textstyle\sum_n a_n^2 = 2\varrho^2+2 = 2\,r_y+2 = 4{,}32 ,\\
  c_1 &= (2\varrho-1)\,e^{\mathrm{i}\,98^\circ} ,\qquad |c_1| = 1{,}1540659 ,\\
  c_2 &= 2\varrho\,e^{\mathrm{i}\,16^\circ} ,\qquad\;\;\;\; |c_2| = 2{,}1540659 ,\\
  c_3 &= \varrho^2\,e^{\mathrm{i}\,114^\circ} ,\qquad\;\;\, |c_3| = 1{,}16 ,
\end{align}
und damit die ganze Einhüllende in einer Zeile:
\begin{empheq}[box=\fbox]{equation}
  |A(t)|^2 = 4{,}32 + 2(2\varrho-1)\cos(\theta+98^\circ)
             + 4\varrho\cos(2\theta+16^\circ)
             + 2\varrho^2\cos(3\theta+114^\circ) .
  \label{eq:yabs2}
\end{empheq}
Zum Vergleich die Rampe $b=38^\circ$: Dort ist das mittlere Paar
gleichgerichtet, $|c_1| = 2\varrho+1 = 3{,}154$ statt $1{,}154$, und alle
drei Kosinus erreichen gleichzeitig $+1$ -- das Maximum ist dann
$(\sum a_n)^2$.

\paragraph{Schritt 4: Der Nenner.}
Über eine volle Periode $T_\mathrm{env}$ durchläuft $\theta$ genau
$[0,2\pi)$, jeder Kosinus in Gl.~\eqref{eq:yabs2} macht $k$ volle
Schwingungen und mittelt zu null. Es bleibt
\begin{equation}
  \langle|A|^2\rangle = c_0 = 4{,}32,
  \qquad
  \mathrm{rms}|A| = \sqrt{4{,}32} = 2{,}0784610 .
\end{equation}
Die Phasen $98^\circ$, $16^\circ$, $114^\circ$ stehen nur \emph{im}
Kosinus und können daran nichts ändern. Mit dem falschen Fenster $1/\Delta f$
käme für diesen Phasensatz $1{,}1264$ heraus.

\paragraph{Schritt 5: Der Zähler.}
Gl.~\eqref{eq:yabs2} auf $4\cdot10^6$ Punkten über $\theta\in[0,2\pi)$
maximiert:
\begin{equation}
  \max|A|^2 = 10{,}4190173 ,\qquad \max|A| = 3{,}2278503 ,
\end{equation}
identisch mit der Handrechnung aus Abschnitt~\ref{sec:handcalc}. Das Maximum
wird pro Periode \emph{zweimal} angenommen, bei $\theta = 192{,}2^\circ$
($t=\SI{4.3288}{\micro\second}$) und $\theta = 331{,}8^\circ$
($t=\SI{7.4730}{\micro\second}$). Der Grund steht in
Abschnitt~\ref{sec:handcalc}: $G(v)$ ist ungerade, $A\,e^{-\mathrm{i}57^\circ}$
erreicht einmal $+3{,}228$ und einmal $-3{,}228$, im Betrag dasselbe.

\paragraph{Schritt 6: Crestfaktor.}
\begin{align}
  \frac{\max|A|}{\mathrm{rms}|A|} &= \frac{3{,}2278503}{2{,}0784610} = 1{,}5530002 ,\\
  \crest_y &= \sqrt2\cdot1{,}5530002 = \mathbf{2{,}1962739} ,\\
  \crest_y^2 &= 4{,}8236 \;\;\hat{=}\;\; \SI{6.83}{\deci\bel}\ \text{PAPR}.
\end{align}
Zur Einordnung zwischen den Schranken aus Gl.~\eqref{eq:sqrt2n}:
\begin{center}
\begin{tabular}{@{}lccc@{}}
  \toprule
  & $\crest_y$ & PAPR & $P_\mathrm{mittel}\le P_{1\mathrm{dB}}/\crest^2$\\
  \midrule
  Untergrenze, ein Ton & $\sqrt2 = 1{,}4142$ & \SI{3.01}{\deci\bel} & --\\
  frei optimiert, Zentrierung verletzt & $1{,}7661$ & \SI{4.94}{\deci\bel} & --\\
  \textbf{Arbeitspunkt, $\gamma=180^\circ$} & $\mathbf{2{,}1963}$ & \SI{6.83}{\deci\bel} & \SI{0.825}{\watt}\\
  Rampe, $\gamma=0$ (z.\,B.\ alle Phasen null) & $2{,}8265$ & \SI{9.03}{\deci\bel} & \SI{0.498}{\watt}\\
  \bottomrule
\end{tabular}
\end{center}
Die Phasenwahl bringt gegenüber der Rampe einen Faktor $1{,}656$ in der
erlaubten mittleren Leistung, \SI{2.2}{\deci\bel}.

\paragraph{Schritt 7: Gegenprobe am vollen HF-Signal.}
Zum Schluss ohne Einhüllende: $s(t) = \sum_m a_m\cos(2\pi f_m t + \varphi_m)$
mit dem \SI{100}{\mega\hertz}-Träger, $3\cdot10^7$ Stützstellen über
$T_\mathrm{env}$ (etwa 37 Punkte pro Trägerperiode):
\begin{center}
\begin{tabular}{@{}lrr@{}}
  \toprule
  & volles Signal & Einhüllende\\
  \midrule
  $\langle s^2\rangle$ bzw.\ $\tfrac12\langle|A|^2\rangle$ & $2{,}159544$ & $2{,}160000$\\
  $\max|s|$ bzw.\ $\max|A|$ & $3{,}227847$ & $3{,}227850$\\
  $\crest$ & $2{,}196503$ & $2{,}196274$\\
  \bottomrule
\end{tabular}
\end{center}
Die relative Abweichung von $1\cdot10^{-4}$ kommt fast ganz aus
$\langle s^2\rangle$: Der Rest des Terms mit doppelter Trägerfrequenz,
Gl.~\eqref{eq:carrierrest}, ist über $T_\mathrm{env}=\SI{8.108}{\micro\second}$
auf $1/(2\pi f_\mathrm{c}T) \approx 2\cdot10^{-4}$ beschränkt und liegt darunter.
Der Crestfaktor der Einhüllenden ist also auf vier Stellen der
Crestfaktor des echten Signals.
...
\begin{tabular}{ll}
  ...
  $P_{1\mathrm{dB}}$ & $+\SI{36}{dBm}$ (einzige Ausgangsleistungsangabe)\\
  OIP3 & $+\SI{49}{dBm}$\\
  \midrule
...
Damit
\begin{equation}
  P_{1\mathrm{dB}} = \tfrac{1}{1000}\cdot10^{36/10}
                   = \tfrac{1}{1000}\cdot 3981 = \SI{3.98}{\watt}.
\end{equation}
Eine Sättigungsleistung nennt das Datenblatt \emph{nicht}; spezifiziert sind
allein Verstärkung, $P_{1\mathrm{dB}}$ und OIP3. Das „5W`` im Typennamen ist
eine Familienbezeichnung, keine garantierte lineare Ausgangsleistung.
(In der Literatur findet sich für dieses Modell gelegentlich
$+\SI{39}{dBm}$ „at 1\,dB compression``; das sind \SI{3}{\deci\bel} zu viel
und im Leistungsbudget ein Faktor zwei.)
...
also ist $P_{1\mathrm{dB}}$ die richtige und zugleich die einzige
spezifizierte Grenze.
...
\end{align}
Beides liegt \emph{über} dem Kompressionspunkt: die \SI{1.3}{\watt} um
\SI{2.9}{\deci\bel}, die vollen \SI{2}{\watt} um \SI{4.8}{\deci\bel}. Dort
ist von Linearität keine Rede mehr, und die Feldrechnung, die sie voraussetzt,
gilt nicht. Mit dieser Kette sind die AOD-Grenzen also gar nicht linear
ausfahrbar.

\subsection{Die gemessene Kette}\label{sec:messkette}

Die bisherigen Zahlen stammen aus dem Datenblatt. Inzwischen ist die
tatsächliche Kette vermessen (Repository \texttt{Amplifier\_characterization},
Messungen vom 23.\ bis 25.09.2026): Rohde\,\&\,Schwarz FPC1500 mit
Tracking-Generator, $S_{12}$ über \SIrange{60}{300}{\mega\hertz} bzw.\
\SIrange{60}{200}{\mega\hertz}, Eingangspegel \SIrange{-30}{0}{dBm},
Kalibrierung in zwei Stufen (Kabel allein, danach Kabel mit dem
\SI{20}{\deci\bel}-Schutzglied), alle Werte in der Verstärkerebene.

\paragraph{Die Kette im AOD-Betrieb.}
Vor dem Verstärker sitzen, in dieser Reihenfolge, ein
\SI{6}{\deci\bel}-Dämpfungsglied, ein Limiter mit \SI{10}{dBm}
Ansprechschwelle, ein DC-Block und ein Tiefpass mit \SI{190}{\mega\hertz}
Grenzfrequenz. Die Messung \texttt{AODconfig(6dB)} erfasst genau dieses
Vorschaltnetz zusammen mit dem Verstärker.

\begin{center}
\begin{tabular}{@{}lccl@{}}
  \toprule
  Messung & Kanal 0 & Kanal 1 & Bemerkung\\
  \midrule
  Kleinsignalverstärkung, Verstärker allein
    & \SI{35.62}{\deci\bel} & \SI{35.62}{\deci\bel} & Welligkeit \SIrange{0.09}{0.10}{\deci\bel}\\
  Kettenverstärkung im AOD-Betrieb bei \SI{100}{\mega\hertz}
    & \SI{28.37}{\deci\bel} & \SI{28.64}{\deci\bel} & Welligkeit \SIrange{0.02}{0.03}{\deci\bel}\\
  Vorschaltverlust (Differenz)
    & \SI{7.25}{\deci\bel} & \SI{6.98}{\deci\bel} & \SI{6}{\deci\bel} Pad plus Rest\\
  Linearität (Steigung des Fits)
    & $0{,}999$ & $0{,}999$ & bis \SI{28.3}{dBm} aus\\
  $P_{1\mathrm{dB}}$ am Ausgang, \SI{100}{\mega\hertz}
    & $+\SI{40.42}{dBm}$ & $+\SI{40.35}{dBm}$ & \SI{11.0}{\watt} bzw.\ \SI{10.8}{\watt}\\
  $P_{1\mathrm{dB}}$ am Eingang
    & $+\SI{6.22}{dBm}$ & $+\SI{6.23}{dBm}$ & Streuung über $f$: \SI{0.13}{\deci\bel}\\
  \bottomrule
\end{tabular}
\end{center}

\paragraph{Drei Vorbehalte.}
Erstens ist $P_{1\mathrm{dB}}$ nur mit vorgeschaltetem Vorverstärker
(ZX60-3018G-S+) erreichbar, weil der Generator des FPC1500 bei \SI{0}{dBm}
endet; der Kompressionspunkt liegt damit am oberen Rand des Messbereichs.
Zweitens hängt der Absolutwert an der Kalibrierung des
\SI{20}{\deci\bel}-Schutzglieds (gemessen \SI{20.03}{\deci\bel}, Streuung
\SI{0.18}{\deci\bel}) und an der Pegelgenauigkeit des Analysators. Drittens
ist die Messung ein Einzelton-Experiment; Intermodulation mehrerer Töne ist
nicht erfasst (der FPC1500 hat nur eine Signalquelle, der OIP3-Wert in den
Protokollen ist eine Faustformel, keine Messung).

\paragraph{Was sich gegenüber dem Datenblatt ändert.}
\begin{center}
\begin{tabular}{@{}lcc@{}}
  \toprule
  & Datenblatt & gemessen\\
  \midrule
  Verstärkung & \SI{30}{\deci\bel} & \SI{35.62}{\deci\bel}\\
  $P_{1\mathrm{dB}}$ & $+\SI{36}{dBm} = \SI{3.98}{\watt}$
                     & $+\SI{40.4}{dBm} = \SI{11.0}{\watt}$\\
  \bottomrule
\end{tabular}
\end{center}
Das sind \SI{4.4}{\deci\bel} mehr, ein Faktor $2{,}8$ im Leistungsbudget. Der
in Abschnitt~\ref{sec:realsetup} erwähnte Literaturwert $+\SI{39}{dBm}$ liegt
zwischen beiden und ist der Messung näher als das Datenblatt.

\subsection{Das Leistungsbudget mit den gemessenen Werten}\label{sec:budgetmess}

Gl.~\eqref{eq:pavglimit} bleibt, nur die Zahl darin ändert sich:
\begin{align}
  \text{$x$-Achse:}\quad &\crest_x^2 = 6{,}00
  &&P_\mathrm{mittel,x} \le \frac{11{,}01}{6{,}00} = \SI{1.84}{\watt},\\
  \text{$y$-Achse:}\quad &\crest_y^2 = 4{,}82
  &&P_\mathrm{mittel,y} \le \frac{10{,}83}{4{,}82} = \SI{2.25}{\watt}.
\end{align}

\medskip
\noindent\fbox{\parbox{0.96\textwidth}{\textbf{Damit dreht sich die Aussage
von Abschnitt~\ref{sec:realsetup} um.} Mit dem Datenblattwert war der
Verstärker das Limit (\SI{0.66}{} und \SI{0.83}{\watt}, weit unter den
\SI{2}{\watt} des AOD). Mit den gemessenen \SI{11}{\watt} liegen beide Achsen
\emph{an oder über} der AOD-Grenze: Jetzt begrenzt der AOD, nicht mehr der
Verstärker. Der Crestfaktor bleibt trotzdem entscheidend -- er legt fest,
\emph{ob} man den Betriebspunkt überhaupt erreicht, siehe
Abschnitt~\ref{sec:effmess}.}}
\medskip

\noindent Die drei Grenzen im neuen Bild:
\begin{center}
\begin{tabular}{@{}llcc@{}}
  \toprule
  Grenze & gehört zu & $x$-Achse & $y$-Achse\\
  \midrule
  $P_{1\mathrm{dB}}/\crest^2$ & Verstärker (Spitze) & \SI{1.84}{\watt} & \SI{2.25}{\watt}\\
  \SI{2}{\watt} mittlere HF-Leistung & AOD-Wandler (Wärme) & \SI{2.00}{\watt} & \SI{2.00}{\watt}\\
  $P_\mathrm{s}$, Maximum der Beugung & AOD (Sättigung) & \SI{1.70}{\watt} & \SI{1.62}{\watt}\\
  \bottomrule
\end{tabular}
\end{center}
Die unterste Zeile ist die eigentlich relevante: Mehr als $P_\mathrm{s}$
bringt keine zusätzliche Beugung, sondern weniger.

\subsection{Vom AWG-Pegel zur HF-Leistung}\label{sec:awgpegel}

Bleibt die Frage, was am AWG einzustellen ist. Die Karte ist eine Spectrum
M4i.6622-x8: vier Kanäle, \SI{16}{bit}, bis \SI{625}{\mega\hertz} Abtastrate,
Ausgangsbereich $\pm\SI{0.08}{\volt}$ bis $\pm\SI{2.5}{\volt}$ an \SI{50}{\ohm},
analoge Bandbreite \SI{200}{\mega\hertz}. Betrieben wird sie mit
\SI{600}{\mega\hertz} Abtastrate; die Töne bei \SI{100}{\mega\hertz} liegen weit
unter Nyquist, das Spiegelbild bei \SI{500}{\mega\hertz} fängt der
\SI{190}{\mega\hertz}-Tiefpass der Kette ab.

\paragraph{Was die Karte ausgibt.}
Im Erzeugungsskript wird das Signal als ganzzahliges Vielfaches der
Vollaussteuerung geschrieben, mit einem Skalierungsfaktor $k$ (im Skript
$0{,}2$), und die Karte bekommt über \texttt{channels.amp} die
Spitzenspannung $V_\mathrm{FS}$ bei Vollaussteuerung. Damit ist
\begin{equation}
  V(t) = V_\mathrm{FS}\,k \sum_n a_n \sin(2\pi f_n t+\varphi_n)
       = V_\mathrm{FS}\,k\,\bigl|A(t)\bigr|\cos(\dots) ,
\end{equation}
also, mit $R=\SI{50}{\ohm}$ und Gl.~\eqref{eq:crestsimple},
\begin{empheq}[box=\fbox]{align}
  P_\mathrm{mittel}^\mathrm{AWG} &= \frac{(V_\mathrm{FS}k)^2}{2R}\sum_n a_n^2 ,
  &
  V_\mathrm{Spitze} &= V_\mathrm{FS}\,k\,\max_t|A| = \crest\,\sqrt{P_\mathrm{mittel}^\mathrm{AWG}R} ,
  \\
  \text{kein Clipping:}\quad & k\,\max_t|A| \le 1 ,
  &
  k_\mathrm{max} &= \frac{1}{\max_t|A|}
  = \begin{cases} 0{,}3367 & (x)\\ 0{,}3098 & (y)\end{cases}
\end{empheq}
$k$ und $V_\mathrm{FS}$ wirken nur als Produkt; sinnvoll ist
$k=k_\mathrm{max}$, weil dann die vollen \SI{16}{bit} genutzt werden und
$V_\mathrm{FS}$ direkt die gewünschte Spitzenspannung ist.

\paragraph{Die einzustellenden Pegel.}
Mit der gemessenen Kettenverstärkung (\SI{28.37}{\deci\bel} bzw.\
\SI{28.64}{\deci\bel}) gehört zu jedem Betriebspunkt am AOD genau ein
AWG-Pegel:
\begin{center}
\small
\begin{tabular}{@{}lcccccc@{}}
  \toprule
  & $P_\mathrm{AOD}$ & $P^\mathrm{AWG}_\mathrm{mittel}$ & $V_\mathrm{Spitze}$
  & $V_\mathrm{FS}$, $k=k_\mathrm{max}$ & Spitze Limiter & Spitze Verst.-Ausgang\\
  \midrule
  $x$, $P_\mathrm{s}$      & \SI{1.70}{\watt} & $+\SI{3.94}{dBm}$ & \SI{0.862}{\volt} & \SI{0.862}{\volt} & $+\SI{5.7}{dBm}$ & $+\SI{40.1}{dBm}$\\
  $x$, AOD \SI{2}{\watt}   & \SI{2.00}{\watt} & $+\SI{4.65}{dBm}$ & \SI{0.935}{\volt} & \SI{0.935}{\volt} & $+\SI{6.4}{dBm}$ & $+\SI{40.8}{dBm}$\\
  $y$, $P_\mathrm{s}$      & \SI{1.62}{\watt} & $+\SI{3.45}{dBm}$ & \SI{0.731}{\volt} & \SI{0.731}{\volt} & $+\SI{4.3}{dBm}$ & $+\SI{38.9}{dBm}$\\
  $y$, AOD \SI{2}{\watt}   & \SI{2.00}{\watt} & $+\SI{4.37}{dBm}$ & \SI{0.812}{\volt} & \SI{0.812}{\volt} & $+\SI{5.2}{dBm}$ & $+\SI{39.8}{dBm}$\\
  \bottomrule
\end{tabular}
\end{center}
Drei Reserven lassen sich daran ablesen:
\begin{enumerate}\itemsep0.15em
  \item \textbf{Karte:} \SI{0.86}{\volt} von \SI{2.5}{\volt} Spitze, also
        \SI{9.2}{\deci\bel} Reserve. Die Karte ist nirgends das Limit.
  \item \textbf{Limiter:} Seine Schwelle von \SI{10}{dBm} gilt für die
        \emph{Spitze}, und der Crestfaktor frisst davon
        $10\log_{10}\crest^2 = \SI{7.8}{\deci\bel}$ ($x$) bzw.\
        \SI{6.8}{\deci\bel} ($y$). Es bleiben rund \SI{4}{\deci\bel}
        Reserve -- genug, aber nicht viel. Ein schlechterer Phasensatz auf
        der $y$-Achse ($\crest_y=2{,}83$) kostet davon allein
        \SI{2.2}{\deci\bel}.
  \item \textbf{Verstärker:} bei $P_\mathrm{s}$ noch \SI{0.3}{\deci\bel}
        ($x$) bzw.\ \SI{1.4}{\deci\bel} ($y$) unter $P_{1\mathrm{dB}}$; die
        AOD-Grenze von \SI{2}{\watt} liegt auf der $x$-Achse bereits
        \SI{0.4}{\deci\bel} \emph{über} $P_{1\mathrm{dB}}$.
\end{enumerate}

\paragraph{Wo das Erzeugungsskript derzeit steht.}
Mit \texttt{channels.amp} $=\SI{0.2}{\volt}$, $k=0{,}2$ und gleichen
Tonamplituden liefert die Karte \SI{48}{\micro\watt} ($N=3$) bzw.\
\SI{64}{\micro\watt} ($N=4$), am AOD also \SI{33}{\milli\watt} und
\SI{47}{\milli\watt}. Das sind gut \SI{17}{\deci\bel} unter dem Betriebspunkt
und entspricht einer Beugungseffizienz von wenigen Prozent. Für den
Arbeitspunkt ist \texttt{channels.amp} auf rund \SI{0.86}{\volt} ($x$) und
\SI{0.73}{\volt} ($y$) zu setzen, bei $k=k_\mathrm{max}$ und mit den
Amplituden $[\varrho,1,\varrho]$ bzw.\ $[\varrho,1,1,\varrho]$.
...
\emph{Die Zahlen dieser Tabelle stehen im Datenblattbudget
$P_{1\mathrm{dB}}=\SI{3.98}{\watt}$. Mit der gemessenen Kette
(Abschnitt~\ref{sec:messkette}) verschieben sich alle vier Zeilen nach oben;
die Reihenfolge und die Abstände bleiben, siehe
Abschnitt~\ref{sec:effmess}.}
...
\subsection{Beugungseffizienz mit den gemessenen Grenzen}\label{sec:effmess}

Mit den Zahlen aus Abschnitt~\ref{sec:messkette} ist die Abschätzung der
Beugung neu zu machen. Zwei Modelle stehen dafür zur Verfügung, und sie
unterscheiden sich gerade dort, wo es jetzt darauf ankommt:
\begin{align}
  \text{Testsheet-Anker, Gl.~\eqref{eq:kappa}:}\quad
    &\eta(P) = \sin^2\!\bigl(\kappa\sqrt P\bigr),
    \qquad \kappa = 0{,}9307\,\si{\watt}^{-1/2},\\
  \text{gemessen (Mittenbühler):}\quad
    &\eta(P) = \alpha\,\sin^2\!\Bigl(\frac{\pi}{2}\sqrt{P/P_\mathrm{s}}\Bigr),
    \qquad \alpha = 0{,}92 .
\end{align}
Das erste hat sein Maximum bei \SI{2.85}{\watt}, das zweite bei
$P_\mathrm{s}=\SI{1.70}{\watt}$ ($x$) bzw.\ \SI{1.62}{\watt} ($y$).
Das erste Modell kennt keine Verluste ($\eta\to1$) und setzt das Maximum
oberhalb der AOD-Grenze an, das zweite hat gemessene Verluste und ein
Maximum bei $P_\mathrm{s}$. Unterhalb von \SI{1}{\watt} liegen beide dicht
beieinander; im jetzt erreichbaren Bereich laufen sie auseinander, und das
ist die ehrliche Unsicherheit dieser Abschätzung.

\begin{center}
\footnotesize
\begin{tabular}{@{}lccccc@{}}
  \toprule
  Betriebspunkt & $P_x$ & $P_y$ & $\eta_\mathrm{Profil}$ Testsheet &
  $\eta_\mathrm{Profil}$ gemessen & Laser, \SI{10}{\micro\watt}\\
  \midrule
  Datenblatt, $P_{1\mathrm{dB}}=\SI{3.98}{\watt}$
    & \SI{0.66}{\watt} & \SI{0.83}{\watt} & \SI{26.5}{\percent} & \SI{47.4}{\percent} & \SIrange{21}{38}{\micro\watt}\\
  gemessen, Verstärkergrenze
    & \SI{1.84}{\watt} & \SI{2.25}{\watt} & \SI{87.9}{\percent} & \SI{78.0}{\percent} & \SIrange{11}{13}{\micro\watt}\\
  gemessen, mit AOD-Grenze \SI{2}{\watt}
    & \SI{1.84}{\watt} & \SI{2.00}{\watt} & \SI{85.0}{\percent} & \SI{81.8}{\percent} & \SI{12}{\micro\watt}\\
  \textbf{empfohlen: $P=P_\mathrm{s}$}
    & \SI{1.70}{\watt} & \SI{1.62}{\watt} & \SI{75.3}{\percent} & \SI{84.6}{\percent} & \SIrange{12}{13}{\micro\watt}\\
  \bottomrule
\end{tabular}
\end{center}

\noindent Die letzte Zeile ist der vernünftige Betriebspunkt: Sie liegt bei
beiden Modellen hoch, unter der thermischen AOD-Grenze und -- anders als die
Zeile darüber -- auf der $x$-Achse noch unter $P_{1\mathrm{dB}}$. Statt der
\SI{26.5}{\percent} aus dem Datenblattbudget sind es
\SIrange{75}{85}{\percent}, also etwa ein Faktor $3$. Für die
\SI{0.87}{\micro\watt} eines $\pi$-Pulses im Profil braucht der Laser dann
rund \SI{1.1}{\micro\watt} statt \SI{3.3}{\micro\watt}.

\paragraph{Und hier zahlt der Crestfaktor.}
Der Betriebspunkt $P_x = P_\mathrm{s} = \SI{1.70}{\watt}$ verlangt eine
Spitzenleistung von
\begin{equation}
  P_\mathrm{peak} = \crest_x^2\,P_\mathrm{s} = 6{,}00\cdot1{,}70 = \SI{10.2}{\watt}
                  = +\SI{40.1}{dBm},
\end{equation}
und gemessen sind $P_{1\mathrm{dB}} = +\SI{40.4}{dBm}$. Die Reserve beträgt
\SI{0.3}{\deci\bel}. Mit $\crest=\sqrt2$ wären es \SI{3.4}{\watt}
Spitzenleistung und \SI{5}{\deci\bel} Reserve; mit dem schlechtesten
zentrierten Satz der $y$-Achse ($\crest_y=2{,}83$) bräuchte die $y$-Achse
$7{,}99\cdot1{,}62=\SI{12.9}{\watt}$ und läge \SI{0.8}{\deci\bel}
\emph{über} $P_{1\mathrm{dB}}$.

\medskip
\noindent\fbox{\parbox{0.96\textwidth}{\textbf{Der Crestfaktor entscheidet
jetzt nicht mehr, wie viel Licht man bekommt, sondern ob der optimale
Betriebspunkt überhaupt linear erreichbar ist.} Mit den gemessenen
\SI{11}{\watt} reicht es auf beiden Achsen -- auf der $x$-Achse mit
\SI{0.3}{\deci\bel}, auf der $y$-Achse dank der Phasenwahl mit
\SI{1.4}{\deci\bel}. Ein um \SI{20}{\percent} schlechterer Crestfaktor auf
der $y$-Achse kippt das.}}
\medskip

\paragraph{Was diese Abschätzung weiterhin nicht enthält.}
Die Kompressionskurve ist mit \emph{einem} Ton gemessen; bei drei oder vier
Tönen gleichzeitig wirkt dieselbe Nichtlinearität als Intermodulation und
erzeugt Mischprodukte bei $2f_i-f_j$, die im Profil als zusätzliche Spots
landen. Solange die Spitze unter $P_{1\mathrm{dB}}$ bleibt, sind sie um
mindestens den OIP3-Abstand unterdrückt, gemessen ist das aber nicht. Ebenso
fehlen die akustische Dämpfung über die Kristalllänge, die
Frequenzabhängigkeit von $P_\mathrm{s}$ (bei Mittenbühler über die
volle Bandbreite gemessen, hier über \SI{0.37}{\mega\hertz} konstant) und die
Temperaturdrift des Wandlers.

\section*{Offene Punkte}

\begin{itemize}\itemsep0.2em
  \item Die Kette ist inzwischen gemessen
        (Abschnitt~\ref{sec:messkette}): Verstärkung, Welligkeit, Linearität
        und $P_{1\mathrm{dB}}$ liegen als Einzeltonmessung vor, einschließlich
        Pad, Limiter, DC-Block und Tiefpass. Nicht gemessen sind die
        Intermodulation bei mehreren gleichzeitigen Tönen, die akustische
        Dämpfung im Kristall und die Temperaturdrift des Wandlers.
        Gl.~\eqref{eq:powerbudget} unterstellt weiterhin eine harte
        Spitzengrenze mit linearem Verhalten bis dorthin.
