...
$P_1=\int|u|^2\mathrm d^2r$. For the Gaussian, substituting
$x=2r^2/w_0^2$,
\begin{equation}
...
=\frac{k^2}{4\pi}\,u(r),
\end{equation}
so that, read backwards,
\begin{equation}
\tilde u(\mathbf q)=\int\! u(\mathbf r)\,e^{-i\mathbf q\cdot\mathbf r}\,\mathrm d^2r
=\frac{4\pi}{k^2}\,\Theta\!\left(k-|\mathbf q|\right),
\end{equation}
a \emph{flat} spectrum inside the cutoff $k=2\pi\,\mathrm{NA}/\lambda$ and zero
outside. The prefactor follows from $\mathbf q=0$:
$\tilde u(0)=\int u\,\mathrm d^2r=\frac{4\pi}{k}\int_0^\infty
J_1(kr)\,\mathrm dr=\frac{4\pi}{k^2}$, using $\int_0^\infty J_1(x)\mathrm dx=1$.
...
=\frac{1}{(2\pi)^2}\int\!\left|\tilde u(\mathbf q)\right|^2
