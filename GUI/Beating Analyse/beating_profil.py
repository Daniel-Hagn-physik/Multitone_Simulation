...
WP = dict(
    ...
    r_x=1.0, r_y=1.2,
    offset=100e6, f1=60e-3, f2=750e-3,
    grid_n=200,
...
def profil(wp=WP):
    ...
    return dict(wp=wp, X=X, Y=Y, x=x, y=y, F=F, f_spots=f_spots, phases=phases,
                f0=f0, T0=1.0 / f0 if f0 > 0 else np.nan, maske=maske, idx=idx,
                centers_x=cx, centers_y=cy, rcx=rcx, rcy=rcy,
                beats=B.unique_beat_frequencies(f_spots),
                degen=B.degenerate_groups(f_spots, f0))
...
def rabi_kurve(prof, t0, t_p, n_t=4001, skala=None, raman=None, **kw):
    ...
    opts.update(kw)
    P = np.mean([R.excitation_series(t, I[:, j], raman, **opts)
                 for j in range(I.shape[1])], axis=0)
    return t - t0, P
...
def flaeche_ueber_t0(prof, t_p, n=120, skala=None, raman=None):
    ...
    for t0 in t0s:
        ...
        th = C * np.trapezoid(_I_roh(prof, tt) * skala, tt, axis=0)
