...
def excitation_series_multi(t, I_t, raman, m=0, B_gauss=0.0, compensate=False,
                            I_ref=None, with_shift=True, with_scatter=True,
                            mitteln=True):
    """Wie excitation_series(), aber fuer VIELE Orte gleichzeitig.

    I_t hat die Form (n_t, n_pix) - eine Intensitaetsgeschichte je Ort. Der
    Zeitschritt wird auf alle Orte auf einmal angewandt (numpy statt einer
    Python-Schleife ueber Pixel), was fuer ein GUI den Unterschied zwischen
    Sekunden und Minuten macht.

    Jeder Ort wird EINZELN propagiert und erst am Ende gemittelt. Das ist der
    Punkt: die Intensitaet ist ortsabhaengig, also auch die Rabi-Frequenz, und
    genau diese Streuung frisst den Kontrast. Wer vor der Propagation mittelt,
    rechnet sie weg und bekommt eine viel zu schoene Kurve.
    """
    t = np.asarray(t, dtype=float)
    I_t = np.atleast_2d(np.asarray(I_t, dtype=float))
    if I_t.shape[0] != t.size:
        I_t = I_t.T
    dt = np.diff(t)
    I_mid = 0.5 * (I_t[:-1] + I_t[1:])          # Mittelpunktsregel -> 2. Ordnung
    cm = raman.coeff(m)
    Om = cm["C_rabi"] * I_mid
    de = (cm["C_shift"] * I_mid if with_shift else np.zeros_like(I_mid))
    de = de + zeeman_detuning(m, B_gauss)
    ...
    for j in range(dt.size):
        ...
        ca, sa = np.cos(a / 2), np.sin(a / 2)
        nz = W > 0
        nx = np.zeros(n_pix); nzz = np.zeros(n_pix)
        nx[nz] = Om[j][nz] / W[nz]
        nzz[nz] = de[j][nz] / W[nz]
        cg, ce = ((ca - 1j * sa * nzz) * cg + (-1j * sa * nx) * ce,
                  (-1j * sa * nx) * cg + (ca + 1j * sa * nzz) * ce)
        surv = surv * np.exp(-G[j] * dt[j])
        out[j + 1] = np.abs(ce) ** 2 * surv
    return out.mean(axis=1) if mitteln else out
