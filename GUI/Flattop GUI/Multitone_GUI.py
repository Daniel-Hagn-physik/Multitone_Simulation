...
==================================
Qt-Layouts ordnen alle Bedienelemente automatisch überlappungsfrei an,
unabhängig von Fenstergröße oder Bildschirm-DPI.
...
class FlatMultiToneWindow(QMainWindow):
    ...
    def build_rectangles_and_handles(self):
        ...
        (self.handle_uniformity,) = self.ax_main.plot([], [], "s", color="cyan", markersize=10,
                                                        markeredgecolor="black", picker=6, zorder=6)
    ...
    def full_update(self):
        ...
        self.cache.update({
            ...
            "mid_y_idx": mid_y_idx, "mid_x_idx": mid_x_idx,
        })
        ...
        self.ax_main.set_xlabel("Ort $x$ (µm)")
        self.ax_main.set_ylabel("Ort $y$ (µm)")
        ...
        self.ax_cut_x.set_xlabel("Ort $x$ (µm)", fontsize=8)
        self.ax_cut_x.set_ylabel("Intensität", fontsize=8)
        self.ax_cut_x.set_title("Schnitt entlang x durch Zentrum", fontsize=8)
        ...
        self.ax_cut_y.set_xlabel("Ort $y$ (µm)", fontsize=8)
        self.ax_cut_y.set_ylabel("Intensität", fontsize=8)
        self.ax_cut_y.set_title("Schnitt entlang y durch Zentrum", fontsize=8)
    ...
    def medium_update(self):
        ...
        mid_y_idx = self.cache["mid_y_idx"]
        mid_x_idx = self.cache["mid_x_idx"]
        self.line_cut_x.set_ydata(I_ort[mid_y_idx, :])
        self.line_cut_y.set_ydata(I_ort[:, mid_x_idx])
    ...
    def on_save(self):
        ...
        ax3.set_title("Schnitt entlang x durch Zentrum")
        ...
        ax4.set_title("Schnitt entlang y durch Zentrum")
    ...
    # --------------------------------------------------------
    # Drag & Drop für die Regionen (matplotlib-Events auf dem Qt-Canvas)
    # --------------------------------------------------------
    def on_pick(self, event):
        if event.artist == self.handle_uniformity:
            self.dragging_target = "uniformity"
        elif event.artist == self.handle_crosstalk:
            self.dragging_target = "crosstalk"
